...
class Right extends CI_Controller {
	...
	public function queryAllUser() {
	    ...
	    $data['status'] = 200;
		$data['msg']    = 'success'; 
		$list = $this->mysql_model->get_results(ADMIN,'(1=1) and sid="'.$this->jxcsys['sid'].'" order by roleid');  
		...
		$data['data']['items']        = $v;
		$data['data']['shareTotal']   = $this->mysql_model->get_count(ADMIN,'(1=1) and sid="'.$this->jxcsys['sid'].'"');
	...
	public function queryalluserright() {
		...
		$list = $this->mysql_model->get_results(MENU,'(isDelete=0) order by sortIndex,path'); 
