...
class Employee extends CI_Controller {
	...
	public function staffList(){
		...
		$list = $this->mysql_model->get_results(STAFF,'(isDelete=0)'.$where.'  order by id desc');
	...
	//得到车型所有数据
	public function getModelsAll($where = ""){
		$v = array();
		$data['status'] = 200;
		$data['msg']    = 'success';
		//车型缓存
		$name   = 'ModelsAll'.md5("ModelsAll");
		$result = $this->cache->get($name);
		if($result){
			$list = $result;
		}else{
			$result = $this->data_model->getModelsAll($where,2);
			$this->cache->save($name,$result,$this->cache_time);
			$list = $result;
		}
		//$list = $this->data_model->getModelsAll($where,2);
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert(STAFF,elements(array('name','number','sid'),$data));
			if ($sql) {
				$data['id'] = $sql;
