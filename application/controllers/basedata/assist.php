...
class Assist extends CI_Controller {
	...
	public function catList(){
		...
		$where .=   $type == 'supplytype' ? ' and sid in ('.$this->jxcsys['sid'].',1)' : ' and sid = '.$this->jxcsys['sid'];
    ...
    public function shopSaleList() {
        ...
        $data['msg']    = 'success';
        $v = array(
            0 => array('id'=>1001,'name'=>'全部','number'=>'all'),
            1 => array('id'=>1002,'name'=>'待确认','number'=>'daiqueren'),
            2 => array('id'=>1003,'name'=>'待审核','number'=>'daishenhe'),
            3 => array('id'=>1004,'name'=>'待发货','number'=>'daifahuo'),
            4 => array('id'=>1005,'name'=>'已发货','number'=>'yifahuo'),
            5 => array('id'=>1005,'name'=>'已收货','number'=>'yishouhuo'),
            7 => array('id'=>1005,'name'=>'已取消','number'=>'yiquxiao'),
            8 => array('id'=>1005,'name'=>'已关闭','number'=>'yiguanbi'),
            9 => array('id'=>1005,'name'=>'已付款完成','number'=>'yiwancheng'),
            10 => array('id'=>1005,'name'=>'未付款完成','number'=>'weiwancheng'),
        );
        $data['data']['items']      = $v;
        $data['data']['totalsize']  = 10;
        die(json_encode($data));
    }
	//查询快准分类
	public function kzCatList1(){
		$v = array();
		$data['status'] = 200;
		$data['msg']    = 'success';
		$type   = str_enhtml($this->input->get('typeNumber',TRUE));
		$skey   = str_enhtml($this->input->get('skey',TRUE));
		$where = ' and typeNumber="'.$type.'"';
		$where = $skey ? ' and cat_name like "%'.$skey.'%"' :'';
		$pid  = array_column($this->mysql_model->get_results(SYSCATEGORY_CAT,'(is_leaf=0) '.$where.' order by cat_id'), 'parent_id');
		$list = $this->mysql_model->get_results(SYSCATEGORY_CAT,'(is_leaf=0) '.$where.' order by cat_path');
		foreach ($list as $arr=>$row) {
			$v[$arr]['detail']      = in_array($row['cat_id'],$pid) ? false : true;
			$v[$arr]['id']          = intval($row['cat_id']);
			$v[$arr]['level']       = $row['level'];
			$v[$arr]['name']        = $row['cat_name'];
			$v[$arr]['parentId']    = intval($row['parent_id']);
			$v[$arr]['remark']      = '备注';
			$v[$arr]['sortIndex']   = intval($row['order_sort']);
			$v[$arr]['status']      = intval($row['is_leaf']);
			$v[$arr]['typeNumber']  = 'kztrade';
		}
		$data['data']['items']      = $v;
		$data['data']['totalsize']  = $this->mysql_model->get_count(SYSCATEGORY_CAT,'(is_leaf=0) '.$where.'');
		die(json_encode($data));
	}
	
	public function kzCatList(){
		$v = array();
		$data['status'] = 200;
		$data['msg']    = 'success';
		$type   = str_enhtml($this->input->get('typeNumber',TRUE));
	...
	public function getReturnType(){
		...
		$data['msg']    = 'success'; 
	    $v = array(
			0 => array('id'=>1001,'name'=>'所有状态','number'=>'allStatus'),
			1 => array('id'=>1002,'name'=>'待审核','number'=>'pending'),
			2 => array('id'=>1003,'name'=>'审核中','number'=>'underReview'),
			3 => array('id'=>1004,'name'=>'审核通过','number'=>'reviewPassed'),
			4 => array('id'=>1005,'name'=>'审核未通过','number'=>'reviewUnPassed')
		);
		$data['data']['items']      = $v;
		$data['data']['totalsize']  = 5;
		die(json_encode($data));		  
	...
	public function getPreType(){
		...
		$v = array(
				0 => array('id'=>1001,'name'=>'所有状态','number'=>'allStatus'),
				1 => array('id'=>1002,'name'=>'待审核','number'=>'pending'),
				2 => array('id'=>1003,'name'=>'审核通过','number'=>'reviewPassed'),
				3 => array('id'=>1004,'name'=>'审核未通过','number'=>'reviewUnPassed'),
				3 => array('id'=>1005,'name'=>'已采购','number'=>'hasPurchase')
		);
		$data['data']['items']      = $v;
		$data['data']['totalsize']  = 5;
		die(json_encode($data));
	...
	public function getOrderType(){
	    ...
	    $v = array(
			...
			3 => array('id'=>1004,'name'=>'审核通过','number'=>'reviewPassed'),
			4 => array('id'=>1005,'name'=>'审核未通过','number'=>'reviewUnPassed'),
			5 => array('id'=>1006,'name'=>'出库','number'=>'outku'),
			6 => array('id'=>1007,'name'=>'入库','number'=>'ruku')
		);
		$data['data']['items']      = $v;
		$data['data']['totalsize']  = 7;
		die(json_encode($data));		  
	}
	...
	public function getSalesReturn(){
		...
		$data['msg']    = 'success';
		$v = array(
				0 => array('id'=>1001,'name'=>'全部退货订单','number'=>'all'),
				1 => array('id'=>1002,'name'=>'待退货订单','number'=>'readyReturn'),
				2 => array('id'=>1003,'name'=>'已退货订单','number'=>'hasReturned'),
				3 => array('id'=>1004,'name'=>'已驳回订单','number'=>'hasRejected'),
		);
		$data['data']['items']      = $v;
	...
	public function add(){
		...
		$sql  = $this->mysql_model->insert(CATEGORY,elements(array('name','typeNumber','sid'),$data));
		if ($sql) {
	...
	public function update(){
		...
		$sql = $this->mysql_model->update(CATEGORY,elements(array('name','typeNumber'),$data),'(id='.$data['id'].') and isSelf = 0');
		if ($sql) {
			$this->common_model->logs($success.$data['name']);
	...
	public  function delete() {
		...
		if (count($data)>0) {
			...
			$sql = $this->mysql_model->update(CATEGORY,$info,'(id='.$id.') and isSelf = 0'); 
		    if ($sql) {
