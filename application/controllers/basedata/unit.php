...
class Unit extends CI_Controller {
	...
	public function index(){
	    ...
	    $data['status'] = 200;
		$data['msg']    = 'success'; 
		$list = $this->mysql_model->get_results(UNIT,'(isDelete=0) '.$where.' order by sid desc');  
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['isSelf']   = intval($row['isSelf']);
	...
	public function add(){
		...
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert(UNIT,elements(array('name','default','sid'),$data));
			if ($sql) {
				$data['id'] = $sql;
	...
	private function validform($data) {
		...
		$this->mysql_model->get_count(UNIT,'(isDelete=0) and (name="'.$data['name'].'") and (sid in ('.$data['sid'].',1))'.$where) && str_alert(-1,'单位名称重复');
		if (isset($data['id'])) {
