...
class Log extends CI_Controller {
	...
	public function queryAllUser(){
	    ...
	    $data['status'] = 200;
		$data['msg']    = 'success';                                             
		$data['data']['totalsize']   = $this->mysql_model->get_count(ADMIN,'(1=1) sid = "'.$this->jxcsys['sid'].'"');     
		$list = $this->mysql_model->get_results(ADMIN,'(1=1) sid = "'.$this->jxcsys['sid'].'" order by uid desc');  
		foreach ($list as $arr=>$row) {
