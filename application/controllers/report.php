...
class Report extends CI_Controller {
	...
	public function index() {
		...
		$fund1 = $fund2 = 0;
		$where = ' and a.sid = '.$this->jxcsys['sid'];
		$where2 = ' and sid = '.$this->jxcsys['sid'];
	    $list = $this->data_model->get_account($where2,$where);
		foreach ($list as $arr=>$row) {
		    if ($row['type']==1) {
			    $fund1 += $row['amount'];
			} else {
		...
		//客户欠款
		$contact1 = $contact2 = 0;
		$where = ' and a.sid in ('.$this->jxcsys['sid'].',1)';
		$where2 = ' and sid = '.$this->jxcsys['sid'];
		$list = $this->data_model->get_contact($where2,$where);
		foreach ($list as $arr=>$row) {
		    if ($row['type']==-10) {
		...
		//采购金额
		$list = $this->data_model->get_invoice_info_sum(' and a.transType=150501 and billDate>="'.date('Y-m-1').'" and billDate<="'.date('Y-m-d').'" and a.sid = '.$this->jxcsys['sid'].' group by a.invId');
		$list3 = $this->data_model->get_invoice_info_sum(' and a.transType=150502 and billDate>="'.date('Y-m-1').'" and billDate<="'.date('Y-m-d').'" and a.sid = '.$this->jxcsys['sid'].' group by a.invId');
        $purchase1 = 0;
        $purchase0 = 0;
        $purchase3 = 0;
		foreach ($list as $arr=>$row) {
			$purchase0 += $row['amount']; 
		}
        foreach($list3 as $arr=>$row){
            $purchase3 += $row['amount']; 
        }
       	$purchase1 = $purchase0-(-$purchase3);
		...
		//销售收入 
		$sales1 = $sales2 = 0;
		$list1  = $this->data_model->get_invoice_info_sum(' and billType="SALE" and billDate>="'.date('Y-m-1').'" and billDate<="'.date('Y-m-d').'" and a.sid = '.$this->jxcsys['sid'].' group by a.invId, a.locationId'); 
	...
	public function puDetail_detail() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['invName']       = $row['brand_name']." ".$row['skuId']." ".$row['invName']." ".$row['invSpec'];   
...
// 		$data['data']['total']['unitPrice']   = $sum1>0 ? $sum3/$sum1 : 0;
	...
	public function puDetail_inv() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['buName']        = $row['contactName'];
			$v[$arr]['invNo']         = $row['invNumber'];  
			$v[$arr]['invName']       = $row['brand_name']." ".$row['skuId']." ".$row['invName'];   
			$v[$arr]['spec']          = $row['invSpec']; 
			$v[$arr]['unit']          = $row['mainUnit'];
			$v[$arr]['minNum']        = $row['minNum'];
			$v[$arr]['location']      = $row['locationName']; 
...
// 		$data['data']['userdata']['unitPrice']   = $sum1>0 ? str_money($sum3/$sum1,$this->systems['qtyPlaces']) : 0;
		...
		if ($profit==1) {
			$data['data']['userdata']['cost']        = $sum5;
			$data['data']['userdata']['unitCost']    = $sum4;
			$data['data']['userdata']['saleProfit']      = $sum6;
			$data['data']['userdata']['salepPofitRate']  = round($sum6/$sum3*100,2).'%';
		}
		die(json_encode($data));
	}
...
// 		$data['data']['userdata']['unitPrice']   = $sum1>0 ? $sum3/$sum1 : 0; 
	...
	public function salesDetail_customer() {
		...
		if ($profit==1) {
			$info  = $this->data_model->get_invoice_info_ini('and billDate<="'.$endDate.'" and sid='.$this->jxcsys['sid'].' group by invId');   
		}
		foreach ($list as $arr=>$row) {
		    $sum1 += $qty = $row['qty']>0 ? -abs($row['qty']):abs($row['qty']);   //销售在数据库中是负数 在统计的时候应该是正数
...
// 		$data['data']['total']['unitPrice']   = $sum1>0 ? $sum3/$sum1 : 0; 
        ...
        if ($profit==1) {
			...
			$data['data']['total']['salepPofitRate']  = round($sum6/$sum3*100,2).'%';
		}
		die(json_encode($data));
	}
	...
	public function contactDebt_detail() {
		...
		$where2 .= 			  ' and sid = '.$this->jxcsys['sid'];
		$list = $this->data_model->get_contact($where2,$where. ' order by type desc');
		foreach ($list as $arr=>$row) {
		    $v[$arr]['dbid']        = 0;
			$v[$arr]['debt']        = 0; 
	...
	public function contactDebt_exporter() {
		...
		$where .= $matchCon ? ' and (name like "%'.$matchCon.'%" or number like "%'.$matchCon.'%")' : '';
		$wher2  = 			' and a.sid = '.$this->jxcsys['sid'];
		$data['list'] = $this->data_model->get_contact($wher2,$where. ' order by type desc');
	...
	public function invBalance() {
		...
		//商品分类
		if ($catId > 0) {
			$cid = array_column($this->mysql_model->get_results(CATEGORY, '(1=1) and find_in_set(' . $catId . ',path)'), 'id');
			if (count($cid) > 0) {
				$cid = join(',', $cid);
		...
		if(!empty($storageNo)){
			$stoid = explode(",", $storageNo);
			if(count($stoid)>1){
				$where .= ' and c.locationNo in ('.$storageNo.')';
			}else{
				$where .= ' and c.locationNo = "'.$storageNo.'"';
			}
		}
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
		$where .= 			' and a.sid = '.$this->jxcsys['sid'];
		...
		if(!empty($storageNo)){
			$stoid = explode(",", $storageNo);
			if(count($stoid)>1){
				$where1 .= ' and locationNo in ('.$storageNo.')';
			}else{
				$where1 .= ' and locationNo = "'.$storageNo.'"';
			}
		}
		$where1 .= 			' and sid = '.$this->jxcsys['sid'];
		...
		$v = array();
		$cost   = $this->data_model->get_invoice_info_ini('and billDate<="'.date('Y-m-d').'" and sid =  '.$this->jxcsys['sid'].'  group by invId');   
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['invNo']         = $row['invNumber'];  
			$v[$arr]['invName']       = $row['invNumber'] ." ". $row['brand_name']." ".$row['skuId']." ".$row['invName'];   
			$v[$arr]['spec']          = $row['invSpec']; 
			$v[$arr]['unit']          = $row['mainUnit'];
			$v[$arr]['minNum']        = $row['minNum'];
	...
	public function invBalance_exporter() {
		...
		$where1 .= 			' and sid = '.$this->jxcsys['sid'];
		$data['cost']  = $this->data_model->get_invoice_info_ini('and billDate<="'.date('Y-m-d').'" and sid = '.$this->jxcsys['sid'].' group by invId');   
	...
	public function deliverDetail() {
		...
		$where1 .= 			 ' and a.sid = '.$this->jxcsys['sid'];
		...
		$where2 .= 			 ' and a.sid = '.$this->jxcsys['sid'];
		$data['data']['page']      = $page;
		$data['data']['records']   = 1;   
		...
		foreach ($list1 as $arr=>$row) {
			...
			foreach ($list2 as $arr1=>$row1) {
				...
				if ($row['invId']==$row1['invId']) {
					...
					$v[$arr]['invName']       = $row1['brand_name'] . ' '.$row1['skuId'] .' ' .$row1['invName'];   
					$v[$arr]['spec']          = $row1['invSpec']; 
					$v[$arr]['unit']          = $row1['mainUnit']; 
					$v[$arr]['minNum']        = $row1['minNum'];
					$v[$arr]['entryId']       = ''; 
	...
	public function deliverSummary() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['invNo']         = $row['invNumber'];  
			$v[$arr]['invName']       = $row['brand_name']." ".$row['skuId']." ".$row['invName']." ".$row['minNum'];   
			$v[$arr]['spec']          = $row['invSpec']; 
			$v[$arr]['unit']          = $row['mainUnit']; 
			$v[$arr]['minNum']        = $row['minNum'];
			$v[$arr]['location']      = $row['locationName']; 
	...
	public function deliverSummary_exporter() {
		...
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
		$where .= 			 ' and a.sid = '.$this->jxcsys['sid'];
		$data['info']   = $this->data_model->get_invoice_info_ini('and billDate<="'.$endDate.'" and sid='.$this->jxcsys['sid'].' group by invId');     
...
//		$data['list']   = $this->data_model->get_invoice_info(' and a.billDate<="'.$endDate.'" group by a.invId,a.locationId'); 
	...
	public function bankBalance_exporter() {
	    $this->common_model->checkpurview(108);
	...
	public function fundBalance_detailSupplier() {
		...
		$where2.= $accountNo ? ' and a.number in('.str_quote($accountNo).')' : '';
		$where2 .= 			   ' and a.sid in ('.$this->jxcsys['sid'].',1)';
		...
		$where3 .= 			   ' and a.sid = '.$this->jxcsys['sid'];
		
		$list2 = $this->data_model->get_invoice($where3.' order by a.id');
		foreach ($list1 as $arr=>$row) {
			$v[$arr]['balance']     = $row['amount'];
		    $v[$arr]['billId']      = 0; 
			$v[$arr]['billNo']      = '期初余额'; 
			...
			foreach ($list2 as $arr1=>$row1) {
				...
				if ($row['id']==$row1['buId']) {
					...
					$v[$arr]['transType']   = $row1['transTypeName']; 
					$v[$arr]['type']        = -1;  
					$v[$arr]['expenditure'] = str_money($a2); 
					$v[$arr]['income']      = str_money($a1);  
				}
			}
			$sum3 = $row['amount'] + $sum1 - $sum2;
			$arr  = $arr + $row['id'];
			$v[$arr]['balance']     = str_money($sum3);
		    $v[$arr]['billId']      = 0; 
			$v[$arr]['billNo']      = '小计'; 
			$v[$arr]['date']        = '';
			$v[$arr]['billTypeNo']  = 0;
			...
			$v[$arr]['transType']   = ''; 
			$v[$arr]['type']        = -1;  
			$v[$arr]['income']      = str_money($sum1); 
			$v[$arr]['expenditure'] = str_money($sum2); 
			$sum4 += $sum1;
			$sum5 += $sum2;
			$sum6 += $sum3;
		} 
		$data['data']['total']['balance']      = str_money($sum6);
		$data['data']['total']['billId']       = '';
		$data['data']['total']['billNo']       = '';
		$data['data']['total']['billTypeNo']   = '';
		...
		$data['data']['total']['buName']       = '';
		$data['data']['total']['date']         = '';
		$data['data']['total']['income']       = str_money($sum4);
		$data['data']['total']['expenditure']  = str_money($sum5);
		$data['data']['total']['number']       = '';
		$data['data']['total']['transType']    = '';
		$data['data']['total']['type']         = '';
	...
	public function fundBalance_detail() {
		...
		$where2 .= 			    ' and a.sid in ('.$this->jxcsys['sid'].',1)';
		$list1 = $this->data_model->get_contact('and (billType="SALE" or billType="RECEIPT") and billDate<"'.$beginDate.'" and sid='.$this->jxcsys['sid'] ,$where2.' order by a.id');
		$where3 = $beginDate ? ' and a.billDate>="'.$beginDate.'"' : '';
		$where3 .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
	...
	public function customerBalance_detail() {
        ...
        $v[0]['amount']        = str_money(0);
	    $v[0]['billId']        = 0; 
		$v[0]['billNo']        = '期初余额'; 
		$v[0]['billType']      = 'BAL'; 
		$v[0]['date']          = '';
		$v[0]['disAmount']     = str_money(0);
		$v[0]['entryId']       = 0; 
		$v[0]['inAmount']      = str_money($arrears);   //期初应收款余额
		$v[0]['invName']       = ''; 
		$v[0]['invNo']         = ''; 
		$v[0]['price']         = '';  
		$v[0]['qty']           = '';  
		$v[0]['spec']          = '';
		$v[0]['rpAmount']      = str_money(0);  
		$v[0]['totalAmount']   = str_money(0);  
		$v[0]['transType']     = '';   
		$v[0]['type']          = -1;   
		$v[0]['unit']          = ''; 
		foreach ($list2 as $arr=>$row) {
			...
			$v[$arr]['amount']      = str_money((float)$row['amount']); //应收金额
		    $v[$arr]['billId']      = intval($row['id']); 
			$v[$arr]['billNo']      = $row['billNo']; 
			$v[$arr]['billType']    = $row['billType']; 
			$v[$arr]['date']        = $row['billDate'];
			$v[$arr]['disAmount']   = str_money($row['disAmount']);
			$v[$arr]['entryId']     = 0; 
			$v[$arr]['inAmount']    = str_money($sum1 + $arrears);  //应收款余额
			$v[$arr]['invName']     = ''; 
			$v[$arr]['invNo']       = ''; 
			$v[$arr]['price']       = '';  
			$v[$arr]['qty']         = '';  
			$v[$arr]['spec']        = '';
			$v[$arr]['rpAmount']    = str_money($row['rpAmount']);  
			$v[$arr]['totalAmount'] = str_money($row['totalAmount']);    //销售金额
			$v[$arr]['transType']   = $row['transTypeName'];   
			$v[$arr]['type']        = 1;   
			$v[$arr]['unit']        = ''; 
			if ($showDetail == "true") {
			    foreach ($list3 as $arr1=>$row1) {
					...
					if ($row['id']==$row1['iid']) {
						...
						$v[$arr]['disAmount']    = 0;
						$v[$arr]['entryId']      = 1; 
						$v[$arr]['inAmount']     = str_money($sum1 + $arrears);  
						...
						$v[$arr]['rpAmount']     = 0;  
						$v[$arr]['spec']         = $row1['invSpec'];
						$v[$arr]['totalAmount']  = str_money($row1['amount']);  
						$v[$arr]['transType']    = '';   
						$v[$arr]['type']         = '';   
						$v[$arr]['unit']         = 0; 
					}
		...
		$data['data']['total']['amount']         = str_money($sum2);
		$data['data']['total']['billNo']         = '';
		$data['data']['total']['billTypeNo']     = '';
		$data['data']['total']['billId']         = '';
		$data['data']['total']['billType']       = '';
		...
		$data['data']['total']['buNo']           = '';
		$data['data']['total']['date']           = '';
		$data['data']['total']['disAmount']      = str_money($sum5);
		$data['data']['total']['inAmount']       = str_money($arrears + $sum1);
		$data['data']['total']['entryId']        = '';
		$data['data']['total']['invName']        = '';
		$data['data']['total']['invNo']          = '';
		$data['data']['total']['price']          = '';
		$data['data']['total']['qty']            = '';
		$data['data']['total']['rpAmount']       = str_money($sum4);
		$data['data']['total']['spec']           = '';
		$data['data']['total']['totalAmount']    = str_money($sum3);
		$data['data']['total']['transType']      = '';
		$data['data']['total']['type']           = '';
		$data['data']['total']['unit']           = '';
		$data['data']['list']                    = array_values($v);
	...
	public function customerBalance_exporter() {
	    $this->common_model->checkpurview(111);
	...
	public function supplierBalance_detail() {
		...
		foreach ($list2 as $arr=>$row) {
			...
			$sum5 += $row['disAmount'];            //折扣率
			$v[$arr]['amount']      = str_money((float)$row['amount']);
		    $v[$arr]['billId']      = intval($row['id']); 
			$v[$arr]['billNo']      = $row['billNo']; 
			$v[$arr]['billType']    = $row['billType']; 
			$v[$arr]['date']        = $row['billDate'];
			...
			$v[$arr]['spec']        = '';
			$v[$arr]['rpAmount']    = str_money($row['rpAmount']);  
			$v[$arr]['totalAmount'] = str_money($row['totalAmount']);  
			$v[$arr]['transType']   = $row['transTypeName'];   
			$v[$arr]['type']        = 1;   
			$v[$arr]['unit']        = ''; 
			if ($showDetail == "true") {
			    foreach ($list3 as $arr1=>$row1) {
					...
					if ($row['id']==$row1['iid']) {
						...
						$v[$arr]['disAmount']    = 0;
						$v[$arr]['entryId']      = 1; 
						$v[$arr]['inAmount']     = str_money(str_money($sum1 + $arrears));  
						$v[$arr]['invName']      = $row1['invName'];
						$v[$arr]['invNo']        = $row1['invNumber'];
						$v[$arr]['price']        = $row1['price'];  
						$v[$arr]['qty']          = $row1['amount']>0 ? abs($row1['qty']) : -abs($row1['qty']);  
