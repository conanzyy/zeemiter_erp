...
class Home extends CI_Controller {
	...
	public function index(){
	    //exit;
		$data['uid']      = trim($this->jxcsys['uid']);
		$data['name']     = trim($this->jxcsys['name']);
		$data['username'] = trim($this->jxcsys['username']);
		$data['roleid'] =  $this->jxcsys['roleid'];
		
		$user  = $this->mysql_model->get_row(ADMIN,'(uid='.$data['uid'].')');
		//获取权限
		!empty($user['lever']) && $menus = $this->mysql_model->get_results(MENU,"(1=1) and id in (".$user['lever'].")","module");
		$rights = array();
		foreach((array)$menus as $m){$rights[$m['module']] = true;};
		$data['rights'] = $rights;
