...
class InvTf extends CI_Controller {
	...
	public function tfList(){
		...
		$where = ' and a.transType=103091'; 
		$where .= $matchCon ?  ' and (a.description like "%'.$matchCon.'%" or a.billNo like "%'.$matchCon.'%")' : ''; 
		...
		if ($inLocationId>0 || $outLocationId>0) {
		    $a1 = $this->data_model->get_invoice_info($where.' and a.locationId='.$inLocationId.' and qty>0 group by iid');  
			$a2 = $this->data_model->get_invoice_info($where.' and a.locationId='.$outLocationId.' and qty<0 group by iid');  
		...
		foreach ($list as $arr=>$row) {
		    foreach ($info as $arr1=>$row1) {
			    if ($row1['iid']==$row['id']) {
				    if ($row1['qty']>0) {
					    ...
					    $inAreaName[$row['id']][]      = $row1['areaName'];
					} else {
					    $outLocationName[$row['id']][] = $row1['locationName'];
					    $outAreaName[$row['id']][]     = $row1['areaName'];
					}
				}
			}
			...
			$v[$arr]['outAreaName']    	   = $outAreaName[$row['id']];
			$v[$arr]['inAreaName']         = $inAreaName[$row['id']];
		}
		$data['data']['rows']        = $v ;
	...
	public function exportInvTf(){
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['inAreaId']    	= $row['areaNo'];
			$v[$arr]['inAreaName']  	= $row['areaName'];
			$v[$arr]['description']     = $row['description'];
		}
		$list = $this->data_model->get_invoice_info($where.' and qty<0  order by id'); 
		foreach ($list as $arr=>$row) {
			$v[$arr]['outAreaId']    	= $row['areaNo'];
			$v[$arr]['outAreaName']  	= $row['areaName'];
	...
	public function area(){
		...
		if($areaId){
			$sto = $areaStorage[$invId][$locationName][$areaId];
			$areaInfo = $this->db->query('select * from '.STORAGE_AREA.' where id = '.$areaId)->row_array();
			$areaInfo['num'] = $sto ? $sto : 0;
			 ...
			 if (is_array($data['entries'])) {
			     foreach ($data['entries'] as $arr=>$row) {
				     if (intval($row['invId'])>0) {
						 ...
						 $s[$arr]['sid']             = $v[$arr]['sid']             = $info['sid'];
						 $s[$arr]['billNo']          = $v[$arr]['billNo']          = $info['billNo'];
						 $s[$arr]['billDate']        = $v[$arr]['billDate']        = $info['billDate'];
						 $s[$arr]['invId']           = $v[$arr]['invId']           = $row['invId'];
						 $s[$arr]['skuId']           = $v[$arr]['skuId']           = $row['skuId'];
						 ...
						 $v[$arr]['areaId']			 = intval($row['inLocationAreaId']);
						 $v[$arr]['qty']             = abs($row['qty']); 
						 $v[$arr]['entryId']         = 1;
						 $s[$arr]['areaId']			 = intval($row['outLocationAreaId']);
     ...
     * @param unknown $data
     */
    private function storage_info($data,$flag = "+"){
    	$cur_time = time();
    	...
    	foreach ($data['entries'] as $arr=>$row) {
    		...
    		if($flag == "+"){
    			$s[$arr]['areaId']    	  = intval($row['inLocationAreaId']);
    			$s[$arr]['storageId']     = $row['inLocationId'];
    		}else{
    			$s[$arr]['areaId']    	  = intval($row['outLocationAreaId']);
    			$s[$arr]['storageId']     = $row['outLocationId'];
    		}
    	...
    	foreach ($s as $arr=>$row) {
    		...
    		if(empty($rs)){
    			try {
    				...
    				$this->mysql_model->update(GOOODS_STORAGE.$this->jxcsys['sid'],' num = num'.$flag.$row['num'],$where);
    			}
    		}else{
    			$this->mysql_model->update(GOOODS_STORAGE.$this->jxcsys['sid'],' num = num'.$flag.$row['num'],$where);
		...
		if (count($data)>0) {
			...
			foreach ($list as $arr=>$row) {
			    if (intval($row['invId'])>0) {
					...
					$v[$arr]['inLocationId']    = $row['locationId'];
					$v[$arr]['inLocationName']  = $row['locationName'];
					$v[$arr]['inLocationAreaId']= $row['areaNo'];
                    $v[$arr]['inLocationArea'] 	= $row['areaName'];
                    $v[$arr]['skuId']  			= $row['skuId'];
				}
			}
			$list = $this->data_model->get_invoice_info('and (iid='.$id.') and qty<0 order by id'); 
			foreach ($list as $arr=>$row) {
                $v[$arr]['outLocationAreaId']   = $row['areaNo'];
				$v[$arr]['outLocationArea']  	= $row['areaName'];
				$v[$arr]['outLocationId']   = $row['locationId'];
				$v[$arr]['outLocationName'] = $row['locationName'];
			}
			$info['data']['entries']   = $v;
		...
		if (strlen($data)>0) {
			 ...
			 if (is_array($data['entries'])) {
			     foreach ($data['entries'] as $arr=>$row) {
                     ...
                     $s[$arr]['sid']          	 = $v[$arr]['sid']             = $invoice['sid'];
					 $s[$arr]['billDate']        = $v[$arr]['billDate']        = $info['billDate'];
					 $s[$arr]['invId']           = $v[$arr]['invId']           = $row['invId'];
					 ...
					 $v[$arr]['entryId']         = 1;
					 $s[$arr]['locationId']      = intval($row['outLocationId']);
                     $v[$arr]['sid']             = $v[$arr]['sid']          = $invoice['sid'];
					 $s[$arr]['qty']             = -abs($row['qty']);  
					 $s[$arr]['entryId']         = 2;
				} 
				if (isset($s) && isset($v)) {
    ...
    public function toPdf() {
		...
		if (count($data)>0) { 
			...
			$list = $this->data_model->get_invoice_info('and (iid='.$id.') and qty<0 order by id'); 
			foreach ($list as $arr=>$row) {
				$v[$arr]['outinAreaId']    	= $row['areaNo'];
                $v[$arr]['outinAreaName']  	= $row['areaName']; 
                $v[$arr]['outLocationId']       = $row['locationId'];
				$v[$arr]['outLocationName']     = $row['locationName'];
			}
	...
	private function validform($data) {
		...
		if (is_array($data['entries'])) {
			...
			foreach ($item as $arr=>$row) {
				...
				intval($row['outLocationAreaId']) == intval($row['inLocationAreaId']) && str_alert(-1,'调出仓库货位不能与调入仓库货位相同！');
                ...
                intval($row['outType']) !== intval($row['inType']) && str_alert(-1,'仓库属性不同! ');

				//库存判断
				if ($system['requiredCheckStore']==1) {  
					if (isset($inventory[$row['invId']][$row['outLocationId']][$row['outLocationAreaId']])) {
						$isEx = $this->mysql_model->get_row(INVOICE_INFO,'(isDelete=0) and billNo="'.$data['billNo'].'" and  invId='.$row['invId'],'qty');
						if(empty($isEx)){
							$inventory[$row['invId']][$row['outLocationId']][$row['outLocationAreaId']] < (float)$row['qty'] && str_alert(-1,$row['outLocationName']." ".$row['invNumber']." ".$row['invName'].'商品库存不足！'.$isEx);
						}else{
							if(($inventory[$row['invId']][$row['outLocationId']][$row['outLocationAreaId']] + abs($isEx)) < (float)$row['qty']){
								str_alert(-1,$row['outLocationName']." ".$row['invNumber']." ".$row['invName'].'商品库存不足！');
							}
						}
					} else {
						str_alert(-1,$row['outLocationName']." ".$row['invNumber']." ".$row['invName'].'商品库存不足.！');
