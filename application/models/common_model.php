...
class Common_model extends CI_Model{
	...
	public function insert_option($key,$val) {
		if (!$this->get_option($key)) {
			...
			$data['option_value'] = json_encode($val,JSON_UNESCAPED_UNICODE);
	...
	public function update_option($key,$val) {
		$data['option_value'] = json_encode($val,JSON_UNESCAPED_UNICODE);
		return $this->mysql_model->update(OPTIONS,$data,'(option_name="'.$key.'") and sid='.$this->jxcsys['sid']);
	}
 
	//获取配置
	public function get_option($key) {
		$option_value = $this->mysql_model->get_row(OPTIONS,'(option_name="'.$key.'") and sid='.$this->jxcsys['sid'],'option_value');
		
		return $option_value ? json_decode($option_value,true) : ''; 
	}
