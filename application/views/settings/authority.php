  ...
  (function($){
    ...
    function initGrid(){
      $('#grid').jqGrid({
        ...
        colModel:[
          ...
          {name:'setting', index:'setting', width:120, align:"center", title:false, formatter: settingFormatter},
    ...
    function settingFormatter(val, opt, row) {
		if (row.admin || row.share === false) {
			...
			return '<div class="operating" data-id="' + row.userId + '"><a class="min-btn btn-default ui-icon-pencil" title="详细设置授权信息" href="../settings/authority_setting?userName=' + row.userName + '&realName='+row.realName+'"> <span class = "fa fa-edit"></span>详细授权信息</a></div>';
		}
    };
    function settingDataFormatter(val, opt, row) {
		if (row.admin || row.share === false) {
			...
			return '<div class="operating" data-id="' + row.userId + '"><a class="min-btn btn-default ui-icon-pencil" title="详细设置授权信息" href="../settings/authority_setting_data?userName=' + row.userName + '&realName='+row.realName+'"> <span class = "fa fa-edit"></span>详细授权信息</a></div>';
		}
    };
