...
<script>
  var urlParam = Public.urlParam(), userName = urlParam.userName,realName = urlParam.realName, curGroup;
  var height = Public.setGrid().h;
  var width = Public.setGrid().w;
  var relation = {
  ...
  function groupFmatter(val, opt, row){
	if(curGroup !== val){
		return '<label><input class="group" type="checkbox" id="'  + val + '"><span class="text"></span></label>';
	} else {
		return '';
	};
  };
  ...
  function rightFmatter(val, opt, row){
	var html_str = '<label><input type="checkbox" class="ckbox" data-for="' + row.fobjectid + '" data-id="' + row.frightid + '"';
	if(row.faction === '查询') {
		html_str = html_str + 'data-view="true"';
	};

	var $over = '<span class="text"></span></label>'
	if(val > 0){
		return html_str + ' checked="checked">' + $over;
	} else {
		return html_str + '>' + $over;
	};
