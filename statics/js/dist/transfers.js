var curRow, curCol, curArrears, loading, urlParam = Public.urlParam(),
	SYSTEM = parent.SYSTEM,currRowId,AreainId,
	...
	THISPAGE = {
		...
		getArea:function(a){
			...
			var areaOutInfo = $("#" + a).data('strongAreaoutInfo') || {};
			var areaHtml = "";
			if(storageInfo){
			...
			var params = {invId:goodsInfo.id,locationId:locationId,areaId:areaOutInfo.id,num:rowData.qty,id:originalData.id};
			Public.ajaxPost("../scm/invTf/area", params, function (res) {
				$("#grid").jqGrid('setRowData', a, {outLocationArea: res['areaInfo']['area_name'], outLocationAreaId:res['areaInfo']['num']});
				$("#" + a).data('strongAreaoutInfo',{id:res['areaInfo']['id'],name:res['areaInfo']['area_name']});
			});
		...
		loadGrid: function(a) {
			...
			function h() {
				$("#initCombo").append($(".storageAuto").val(""))
                    ...
                    /*------------------调出仓库区域------------------*/
			function ll(a,b,c) {
				var obj = $(".strongAreaout")[0];
				var id = b.id.split("_outLocationArea");
				currRowId = id[0];
				return obj;
			}
			function mm(a, b, c) {
				if ("get" === b) {
					if ("" !== $(".strongAreaout").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr");
					return d.removeData("strongAreaoutInfo"), ""
				}
				"set" === b && $("input", a).val(c)
			}
			function nn() {
				$("#initCombo").append($(".strongAreaout").val(""))
			}
			/*------------------调出仓库区域------------------*/
			...
			/*----------------------调入仓库区域--------------------------*/
            function xx(a,b,c) {
				var obj = $(".strongAreain")[0];
				var id = b.id.split("_inLocationArea");
				AreainId = id[0];
				return obj;
			}
			function yy(a, b, c) {
				if ("get" === b) {
					if ("" !== $(".strongAreain").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr");
					return d.removeData("strongAreainInfo"), ""
				}
				"set" === b && $("input", a).val(c)
			}
			function zz() {
				$("#initCombo").append($(".strongAreain").val(""))
			}
            /*----------------------调入仓库区域--------------------------*/
			function l() {
				var a = $(".unitAuto")[0];
			...
			function u(a) {
				...
				if (b) {
					...
					d && (THISPAGE.calTotal(),THISPAGE.getArea(a));
				}
			}
			var v = this,
				w = (new Date).format();
			...
			var z = "grid",
				A = [{
					...
					editoptions: {
						...
						custom_value: d,
						handle: e,
						trigger: "ui-icon-storage-search"
					}
				}, {
					name: "skuId",
					label: "属性ID",
						...
						for (var d = 0; c > d; d++) {
							...
							Business.cacheManage.getGoodsInfoByNumber(g.number, function(a) {
								g.isSerNum = a.isSerNum, g.isWarranty = f.isWarranty = a.isWarranty, g.safeDays = f.safeDays = a.safeDays, g.id = f.invId, $("#" + e).data("goodsInfo", g).data("storageInfo", {
									...
									unitId: f.unitId,
									name: f.mainUnit
								}).data("strongAreainInfo",{
                                        id: f.inLocationAreaId,
                                        name: f.inLocationArea
                                }).data("strongAreaoutInfo",{
                                        id: f.outLocationAreaId,
                                        name: f.outLocationArea
                                })
				...
				afterEditCell: function(a, b, c, d, e) {
					function f() {
						...
						if (b) {
							...
							b = $.extend(!0, {}, b), b.mainUnit = c.mainUnit, b.unitId = c.unitId, b.qty = c.qty, b.price = c.price, b.discountRate = c.discountRate, b.deduction = c.deduction, b.amount_old = c.amount, b.taxRate = c.taxRate, b.tax = c.tax, b.taxAmount = c.taxAmount, b.inLocationId = c.inLocationId, b.inLocationName = c.inLocationName, b.outLocationId = c.outLocationId, b.outLocationName = c.outLocationName,b.outLocationAreaId = c.outLocationAreaId,b.outLocationAreaName=c.outLocationAreaName,b.inLocationAreaId=c.inLocationAreaId,b.inLocationAreaName=c.inLocationAreaName, $("#" + a).data("goodsInfo", b)
						}
					}
					...
					if ("outLocationArea" === b) {
						$("#" + d + "_outLocationArea", "#grid").val(c), 
						THISPAGE.strongAreaoutCombo.selectByText(c);
					}
					...
					if ("goods" === b) {
						...
						var e, f = function(b) {
								SYSTEM.ISSERNUM && b.isSerNum ? (Business.serNumManage({
                                    ...
                                    id: b.inLocationAreaId,
                                    name: b.inLocationAreaName
                                }).data("strongAreaoutInfo",{
                                    id: b.outLocationAreaId,
                                    name: b.outLocationAreaName
		...
		initCombo: function() {
			...
			this.outStorageCombo = $(".storageAuto").combo({
				data: function() {
					if (defaultPage.SYSTEM.storageInfo) {
						for (var a = [], b = 0; b < defaultPage.SYSTEM.storageInfo.length; b++) {
				...
				callback: {
					onChange: function (data) {
						if(data) {
							...
							parentTr.data('storageInfo', {id: data.id, name: data.name ,types:data.type});
							var h = $("#grid").jqGrid("getRowData", parentTr.attr('id'));
							var k = $("#" + parentTr.attr('id')).data("storageInfo");
							...
							if (defaultPage.SYSTEM.storageAreaInfo) {
								...
								parentTr.data('strongAreaoutInfo', { id: aa.id, name: aa.name });
							}
			...
			this.strongAreaoutCombo = $(".strongAreaout").combo({
				data: function(i) {
					if(currRowId == null) return [];
					var data = $("#"+currRowId).data("storageInfo");
					if(data == null){
						parent.Public.tips({
							type: 1,
							content: "请先选择调出仓库 ！"
				...
				callback: {
					onChange: function(a) {
						var b = this.input.parents("tr"),
							c = b.data("strongAreaoutInfo");
						c || (c = {}), a && (c.id = a.id, c.name = a.name), b.data("strongAreaoutInfo", c);
						if (a) {
							THISPAGE.getArea(b.attr('id'));
						}
					}
				}
			}).getCombo()
			,this.inStorageCombo = $(".inStorage").combo({
				data: function() {
					if (defaultPage.SYSTEM.storageInfo) {
				...
				callback: {
					onChange: function(data) {
						if(data) {
							var parentTr = this.input.parents('tr');
							parentTr.data('inStorage', {id: data.id, name: data.name,types : data.type});
							alert(parentTr.data('inStorage').id);
						}
					}
				}
			}).getCombo(),
			this.strongAreainCombo = $(".strongAreain").combo({
				data: function(i) {
					if(AreainId == null) return [];
					var data = $("#"+AreainId).data("inStorage");
					if(data == null){
						parent.Public.tips({
							type: 1,
							content: "请先选择调入仓库 ！"
						...
						return [];
                                       ...
                                       // alert(defaultPage.SYSTEM.storageAreaInfoin);
					if (defaultPage.SYSTEM.storageAreaInfo) {
						var areas =  defaultPage.SYSTEM.storageAreaInfo[data.id];
						return areas;
					}
				...
				callback: {
					onChange: function(data) {
						if(data){
							var parentTr = this.input.parents("tr");
							parentTr.data('strongAreainInfo', { id: data.id, name: data.name});
						}
					}
				}
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				setTimeout(function() {
					b.val();
					if(b.hasClass("unitAuto")){
						b.trigger("click");
					}else if(b.hasClass("inStorage")){
						(a.inStorageCombo.active = !0, a.inStorageCombo.doQuery());
					}else if(b.hasClass("strongAreaout")){
						(a.strongAreaoutCombo.active = !0, a.strongAreaoutCombo.doQuery());
					}else if(b.hasClass("strongAreain")){
                                                (a.strongAreainCombo.active = !0, a.strongAreainCombo.doQuery());
                                        }else{
						(a.outStorageCombo.active = !0, a.outStorageCombo.doQuery());
					}
			...
			for (var a = [], b = $("#grid").jqGrid("getDataIDs"), c = 0, d = b.length; d > c; c++) {
				...
				if ("" !== g.goods) {
					var h = $("#" + f).data("goodsInfo"),
						...
						j = $("#" + f).data("inStorage"),
						ps = $("#" + f).data("strongAreaoutInfo"),//调出区域
						os = $("#" +f).data("strongAreainInfo"),//调入区域
						k = $("#" + f).data("unitInfo") || {};
					...
					e = {
						...
						outType : i.types,
						outLocationAreaId: ps.id,
						outLocationAreaName: ps.name,
						inLocationId: j.id,
						inLocationName: j.name,
						inType: j.types,
						inLocationAreaId: os.id,
						inLocationAreaName: os.name,
						serNumList: l
