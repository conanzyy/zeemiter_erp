...
var THISPAGE = {
	...
	loadGrid: function() {
		function a(a, b, c) {
			var d = '<div class="operating" data-id="' + c.id + '"><span class="min-btn btn-success ui-icon-pencil" title="修改"><span class="fa fa-edit"></span>修改</span> <span class="min-btn btn-warning ui-icon-trash" title="删除"><span class="fa fa-trash"></span>删除</span></span></div>';
			if(c.sourceOrder){
				d = '<div class="operating" data-id="' + c.id + '"><span class="min-btn btn-success ui-icon-pencil" title="查看"><span class="fa fa-edit"></span>查看</span></span></div>';
			}
			return d
		}
		...
		$("#grid").jqGrid({
			...
			gridview: !0,
			multiselect: !0,
			colNames: ["操作", "单据日期", "单据编号", "供应商", "采购金额", "优惠后金额", "已" + c + "款", c + "款状态", "制单人", "审核人","订单来源","订单编号", "备注", "订单来源"],
			colModel: [{
				name: "operating",
				width: 110,
				fixed: !0,
				...
				formatter: function(a){
					if(a){
						return "<a class = 'min-btn order-source' data-number="+a+">"+a+"</a>";
					}
					return "";
				},
				title: !0,
	...
	addEvent: function() {
		...
		$(".grid-wrap").on("click", ".order-source", function(a) {
			if (a.preventDefault(), Business.verifyRight("PO_INSTORAGE")) {
				...
				parent.tab.addTabItem({
					...
					text: '采购订单详情',
					url: "../scm/invOd?action=orderList&number=" + b + "&flag=list",
				})
			}
		});
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			"150502" == queryConditions.transType ? (e = "采购退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#grid").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#grid").jqGrid("getDataIDs"), parent.tab.addTabItem({
				tabid: f,
				text: e,
				url: "../scm/invPu?action=editPur&status=edit&id=" + b + "&flag=list" + d + "&transType=" + queryConditions.transType
			})
		}), $(".grid-wrap").on("click", ".ui-icon-trash", function(a) {
			if (a.stopPropagation(),a.preventDefault(), Business.verifyRight("PU_DELETE")) {
				var b = $(this).parent().data("id");
