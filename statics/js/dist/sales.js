var curRow, curCol, loading, settlePrice = {},CONFIG = parent.CONFIG , SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		initData:function(){
			//得到最新销售价
			Public.ajaxGet(CONFIG.SITE_URL + "/scm/invSa/get_settle_price", {}, function(data){
				if(data.status === 200) {
		...
		getArea:function(a){
			var rowData = $("#grid").jqGrid("getRowData", a);
			var goodsInfo = $("#"+a).data("goodsInfo"); 
			var storageInfo = $("#" + a).data('storageInfo');
			var areaHtml = "";
			if(storageInfo){
				var locationId = storageInfo.id;
			}else{
				var locationId = goodsInfo.locationId;
			}
			var params = {invId:goodsInfo.id,locationId:locationId,num:rowData.qty,transType:originalData.transType,id:originalData.id};
			Public.ajaxPost("../scm/invSa/area", params, function (res) {
				if(originalData.transType == "150602"){
					$("#grid").jqGrid('setRowData', a, {locationArea: res['areaInfo']['area_name'], locationAreaId:res.arr});
				} else {
					if(res.storage>0){
						areaHtml = "<span class='ui-label ui-label-success'>"+res['areaInfo']['area_name']+"库存：" + res['areaInfo']['num'] + '</span>';
					}else {
						areaHtml = "<span class='ui-label ui-label-important'>库存不足,当前库存："+res.cur_num+"</span>";
					}
					$("#grid").jqGrid('setRowData', a, {locationArea: areaHtml, locationAreaId:res.arr});
				}
				$("#" + a).data("areaInfo",res.areaInfo);
			});
			
		},
		init: function(a) {
			"150602" == urlParam.transType ? this.mod_PageConfig = Public.mod_PageConfig.init("salesBack") : this.mod_PageConfig = Public.mod_PageConfig.init("sales"), 
			SYSTEM.isAdmin !== !1 || SYSTEM.rights.AMOUNT_OUTAMOUNT || (hiddenAmount = !0, $("#amountArea").hide()),
			SYSTEM.isAdmin !== !1 ?  SYSTEM.showLowPrice == 0 && (hiddenLowPrice = !0) : !SYSTEM.rights.LOW_SALE_PRICE && (hiddenLowPrice = !0),
			this.initDom(a), 
			this.loadGrid(a), 
			this.initCombo(), 
		...
		initDom: function(a) {
			var b = this;
			if (this.$_customer = $("#customer"),this.$_sourceOrder = $("#sourceOrder"),this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"),this.$_paymentInfo = $("#paymentInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_checkName = $("#checkName"), this.$_customerFree = $("#customerFree"), this.customerArrears = 0, this.$_note.placeholder(), "150602" == originalData.transType) {
			...
			if ("add" !== a.status || a.salesId) var c = ["id", a.salesId],A = ["wayId",a.wayId],B = ["id", a.accId];
			else var c = 0,A = 0,B = 0;
			if (this.salesCombo = Business.billSalesCombo($("#sales"), {
				defaultSelected: c
			}), this.customerCombo = Business.billCustomerCombo($("#customer"), {
				defaultSelected: 0,
				addOptions:{text : '（请选择客户）',value : ''},
				callback: {
					onChange: function(a) {
						if(a){
							$("#customer").data("contactInfo", a);
			...
			})),this.paymentCombo = Business.paymentCombo($("#payments"), {
				width: 112,
				height: 310,
				emptyOptions: !0,
				defaultSelected: A,
				callback: {
					onChange: function(a) {
						if (-1 === this.getValue()) b.chooseAccount();
							...
							c.push({
								wayId: this.getValue(),
							}), b.$_paymentInfo.data("paymentInfo", c).hide()
						}
					}
				}
			});
			var f = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="print2" target="_blank"  class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a id="print2" target="_blank"  class="ui-btn">打印</a>',
				i = "",
				j = "",
				A = '<a id="pri" class="ui-btn">打印3</a><a id="qr" class="ui-btn">发货</a>',
				B = '<a id="th" class="ui-btn">退货</a>',
				k = "";
				this.btn_edit = g;
			...
			if (f += k, this.btn_audit = i, this.btn_view = h, this.btn_reaudit = j, this.btn_tempSave = k, a.id > 0) {
				if (this.$_number.text(a.billNo),this.$_sourceOrder.val(a.sourceOrder), this.$_date.val(a.date), this.$_totalArrears.val(a.totalArrears), this.$_accountInfo.data("accountInfo", a.accounts), -1 === a.accId && (this.$_accountInfo.show(), b.$_payment.attr("disabled", "disabled").addClass("ui-input-dis")), $("#grid").jqGrid("footerData", "set", {
					qty: a.totalQty,
					...
					if(a.from == "qr"){
						if(a.sourceType == 2){
							...
							m = "<span id=groupBtn>" + A + "</span>";
							THISPAGE.disableEditQr();
						}
					}else if(a.from == "th"){
						if(a.sourceType == 2){
							$("#button-row").addClass("hide");
							...
							m = "<span id=groupBtn>" + B + "</span>";
							THISPAGE.disableEditQr();
						}
					}else{
						m = "<span id=groupBtn>" + g + i + "</span>" + l;
					}
			...
			}else billRequiredCheck ? this.$_toolBottom.html("<span id=groupBtn>" + f + i + "</span>") : this.$_toolBottom.html('<span id="groupBtn">' + f + "</span>"), this.$_userName.html(system.realName || ""),  this.$_checkName.parent().hide();
		...
		loadGrid: function(a) {
			...
			function area_e(a, b, c){
				if ("get" === b) {
					if ("" !== $(".areaAuto").getCombo().getValue()) return $(a).val();
					var d = $(a).parents("tr");
					return d.removeData("areaInfo"), ""
				}
				"set" === b && $("input", a).val(c)
			}
			function area_f() {
				$("#initCombo").append($(".areaAuto").val(""))
			}
			...
			function v(a) {
				...
				if (b) {
					//客户类型修改价格
					var salePrice = b.salePrice;
					var c = w.$_customer.data("contactInfo");
					if (c && c.id) {
						var	e = $.parseJSON(b.retailPrice);
						if(e[c.cCategory]){
							salePrice = e[c.cCategory];
						}
					}
					var oprice = '最近无销售';
					if($("#customer").data("contactInfo")){
						$buId = $("#customer").data("contactInfo").id;
						if(settlePrice[b.id]){
							oprice = settlePrice[b.id][$buId] ? Public.numToCurrency(settlePrice[b.id][$buId]) : '最近无销售';
						}
					}
					var f = {
						gid : b.id,
						...
						unitId: b.unitId,
						qty: b.qty || 1,
						lprice: Public.numToCurrency(b.lprice),
						price: salePrice,
						oprice: oprice,
					...
					n && (THISPAGE.calTotal(),THISPAGE.getArea(a));
				}
			}
			var w = this,
				x = (new Date).format();
			...
			var B = [{
				...
				width: 98,
				fixed: !0,
				formatter: billsOper_goods = function (val, opt, row) {
					var delBtn = '';
					if(a.from == null || a.from == ""){
						delBtn = ' <span class = "min-btn ui-icon-trash btn-warning"><span class="fa fa-trash" title="删除行"></span>删除</span>';
					}
					var html_con = '<div class="operating" data-id="' + opt.rowId + '"> '+delBtn+' <span class = "min-btn ui-icon-car btn-info"><span class="fa fa-search" title="详情"></span>详情</span></div>';
					return html_con;
				},
				align: "left"
			}, {
			...
			if(a.from) {
				for(var i = 0; B != null && i < B.length;i++){
					if(a.sourceType != 2 && B[i]['name'] == 'locationName') continue;
					if(a.sourceType != 2 && B[i]['name'] == 'qty') continue;
					if(a.sourceType != 2 && B[i]['name'] == 'price') continue;
					if(a.sourceType != 2 && B[i]['name'] == 'amount') continue;
					B[i]['editable'] = false;
				}
			}
			...
			$("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					if ("price" === b && $("#" + d + "_price", "#grid").val(c),  "locationName" === b && ($("#" + d + "_locationName", "#grid").val(c), THISPAGE.storageCombo.selectByText(c), "150601" == originalData.transType) ,"locationArea" === b && ($("#" + d + "_locationArea", "#grid").val(c), THISPAGE.storageCombo.selectByText(c))) {
						var g = $("#" + a).data("goodsInfo"),
							i = $("#" + a).data("storageInfo") || {},
							ia = $("#" + a).data("areaInfo") || {};
						if (!g || !i.id || ia.id) return;
						if (SYSTEM.ISSERNUM && 1 == g.isSerNum) {
							$("#grid").jqGrid("restoreCell", d, e);
							var h = g.serNumList;
							Business.serNumManage({
				...
				afterSaveCell: function(a, c, d, e, f) {
					...
					case "qty":
						var d = parseFloat(d),
							g = parseFloat($("#grid").jqGrid("getCell", a, f + 3));
							var k = $("#grid").jqGrid("setRowData", a, {
								amount: d * g,
							});
							b(a), k && (THISPAGE.calTotal(),THISPAGE.getArea(a));
						break;
					case "price":
						var d = parseFloat(d),
		...
		priceBatch:function(cInfo){
			var ids = $("#grid").jqGrid("getDataIDs")
			for(var i = 0;i<ids.length;i++){
				var b = $("#grid").jqGrid('getRowData',ids[i]);
				var goodsInfo = $("#"+ids[i]).data('goodsInfo');
				if(b['goods'] == "") continue;
				
				var prices = goodsInfo.retailPrice ? $.parseJSON(goodsInfo.retailPrice) : {};
				if(prices[cInfo.cCategory] != "" && prices[cInfo.cCategory] != null){
					b.price = prices[cInfo.cCategory];
				}
				/*if (b.qty > 0) {
					var g = parseFloat(b.qty),
						h = parseFloat(b.price)
						i = parseFloat(b.discountRate);
					//$.isNumeric(h) && ($.isNumeric(i) ? (b.deduction = b.deduction || g * h * i / 100, b.amount = b.amount || g * h - b.deduction) : b.amount = b.amount || g * h)
				}*/
				var oprice = '最近无销售';
				if($("#customer").data("contactInfo")){
					$buId = $("#customer").data("contactInfo").id;
					if(settlePrice[b.gid]){
						b.oprice = settlePrice[b.gid][$buId] ? Public.numToCurrency(settlePrice[b.gid][$buId]) : '最近无销售';
					}
				}
				...
				if (taxRequiredCheck) {
					var k = b.taxRate,
						l = j * k / 100,
						m = j + l;
					b.tax = b.tax || l, b.taxAmount = b.taxAmount || m
				}
				var n = $("#grid").jqGrid("setRowData", ids[i], {oprice:b.oprice,price:b.price,amount:b.amount});
				n && THISPAGE.calTotal()
			}
		},
			...
			function b(a, b, c) {
				...
				if(A) {
					for (var i = 0; i < DataId.length; i++) {
						if ($("#grid").jqGrid("getRowData", i).goods == A) {
							return (parent.Public.tips({
								type: 2,
								content: "商品重复，请重新选择！"
							}),d.removeData("goodsInfo"), "")
						}
					}
				}
				if ("get" === b) {
		...
		initCombo: function() {
			var a = this;
			this.areaCombo =Business.areaCombo($(".areaAuto"),{
				callback:{
					onChange: function(data){
						if(data) {
							var parentTr = this.input.parents('tr');
							parentTr.data('areaInfo', {id: data.id, name: data.name});
							$("#grid").jqGrid('setRowData', parentTr.attr('id'), {'locationAreaId':data.id});
						}
					}
				}
			}),
                ...
                callback: {
                    onChange: function(data){
                    	if(data) {
							...
							var h = $("#grid").jqGrid("getRowData", parentTr.attr('id'));
							if (h.goods) {
								THISPAGE.getArea(parentTr.attr('id'));
							}
						}
                    }
                }
		...
		disableEdit: function() {
			this.customerCombo.disable(), this.salesCombo.disable(), this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_note.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_discountRate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_deduction.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_payment.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_customerFree.attr("disabled", "disabled").addClass("ui-input-dis"), this.accountCombo.disable(), this.paymentCombo.disable(),$("#grid").jqGrid("setGridParam", {
				cellEdit: !1
			}), this.editable = !1
		},
		disabledOper:function(){


		},
		disableEditQr: function() {
			$(".ui-icon-plus").hide();
			this.customerCombo.disable(),this.$_customer.find('input').addClass("ui-input-dis") , this.$_date.attr("disabled", "disabled").addClass("ui-input-dis"), $("#userName").parent().hide();$("#modifyTime").parent().hide(), this.$_discountRate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_deduction.attr("disabled", "disabled").addClass("ui-input-dis"), $("#grid").jqGrid("setGridParam", {
				cellEdit: !1
			}), this.editable = !1
		},
		enableEdit: function() {
			disEditable || (this.salesCombo.enable(), !hideCustomerCombo && this.customerCombo.enable(), this.$_date.removeAttr("disabled").removeClass("ui-input-dis"), this.$_note.removeAttr("disabled").removeClass("ui-input-dis"), this.$_discountRate.removeAttr("disabled").removeClass("ui-input-dis"), this.$_deduction.removeAttr("disabled").removeClass("ui-input-dis"), this.$_payment.removeAttr("disabled").removeClass("ui-input-dis"), this.$_customerFree.removeAttr("disabled").removeClass("ui-input-dis"), this.accountCombo.enable(),this.paymentCombo.disable(), $("#grid").jqGrid("setGridParam", {
				cellEdit: !0
			}), this.editable = !0)
		},
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				this.select()
			}),$('.grid-wrap').on('click', '.ui-icon-car', function(e){
				e.preventDefault();
				var goodsInfo = $(this).closest('tr').data('goodsInfo');
				var name = $("#customer").data("contactInfo");
				if(!goodsInfo){
				...
				THISPAGE.forSearch(goodsInfo.id,originalData.transType,name.name);
			}),$('.grid-wrap').on('click', '.ui-icon-set', function(e){
				...
				for (var b = [], c = $("#grid").jqGrid("getDataIDs"), d = 0, e = c.length; e > d; d++) {
					...
					$.dialog({
						...
						height: 400,
						title: '配置',
						content: 'url:../scm/invSa/set?invId='+i.id+'&locationId='+k.id+'&name='+k.name,
						cancel: true,
						lock: true,
						cancelVal: '关闭'
					});
			...
			}),$(".wrapper").on("click", "#pri", function(b) {
				b.preventDefault();
				var c = $(this),
					d = THISPAGE.getPostData(),
					e = JSON.stringify(d);
				$('#postData').val(e);
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"),$('#form').submit());
			}), $(".wrapper").on("click", "#qr", function(b) {
				b.preventDefault();
				var c = $(this),
					d = THISPAGE.getPostData();
				var target = parent.getIframe("shopSalesQuery-shopSalesQueryALL");
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invSa/shopWait", {
					postData: JSON.stringify(d)
				}, function(b) {
					200 === b.status ? (parent.Public.tips({
						content: "发货成功！"
					}),target[0].contentWindow.refirsh(),parent.closeTab("sales-sales")) : parent.Public.tips({
						type: 1,
						content: b.msg
					})
				}))
			}),$(".wrapper").on("click", "#th", function(b) {
				b.preventDefault();
				var c = $(this),
					d = THISPAGE.getPostData();
				var target = parent.getIframe("shopSalesQuery-salesReturn");
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invSa/returnYes", {
					postData: JSON.stringify(d)
				}, function(b) {
					200 === b.status ? (parent.Public.tips({
						content: "处理成功！"
					}),target[0].contentWindow.refirsh(),parent.closeTab("sales-salesBack")) : parent.Public.tips({
						type: 1,
						content: b.msg
					})
				}))
				...
				if (b.preventDefault(), Business.verifyRight("SA_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					 urlParam.from  == "qr" ? c.ajaxPost("../scm/invSa/add?action=add", {
						postData: JSON.stringify(d)
					}, function(b) {
						200 === b.status ? (a.$_modifyTime.html((new Date).format("yyyy-MM-dd hh:mm:ss")).parent().show(), originalData.id = b.data.id, billRequiredCheck ? a.$_toolBottom.html('<span id="groupBtn">' + a.btn_edit + a.btn_audit + "</span>") : a.$_toolBottom.html('<span id="groupBtn">' + a.btn_edit + "</span>"), parent.Public.tips({
							content: "保存成功！"
			...
			}), $(".wrapper").on("click", "#reAudit", function(b) {1
			...
			}), $(".wrapper").on("click", "#print2", function(a) {
				a.preventDefault(), Business.verifyRight("SA_PRINT") && Public.print({
					title: "销售单列表",
					$grid: $("#grid"),
					pdf: CONFIG.SITE_URL + "/scm/invSa/toPdf?action=toPdf",
					billType: 150602,
					filterConditions: {
						id: originalData.id
					}
				...
				if ("" !== h.goods) {
					...
					if (i) {
						...
						if (!k || !k.id) return parent.Public.tips({
						 ...
						 var ar = $("#" + g).data("areaInfo");
						if (h.locationAreaId == "") return parent.Public.tips({
						 	type: 2,
						 	content: i.name+"未找到货位！"
						}),!1
						...
						f = {
							...
							qty: h.qty,
							price: h.price,
							oprice: !isNaN(h.oprice) ? h.oprice : 0,
							lprice: !isNaN(h.lprice) ? h.lprice : 0,
							discountRate: h.discountRate || 0,
							deduction: h.deduction,
							amount: h.amount,
							locationId: k.id,
							locationName: k.name,
							locationAreaId: h.locationAreaId,
							// locationAreaName: ar.name,
		...
		forSearch : function(id, text,name){
		if(id) {
			$.dialog({
				width: 900,
				height: 410,
				title: '详情',
				content: 'url:../inventory/detail?invId='+id+'&transType='+text+'&name='+name,
		...
		getPostData: function(a) {
			...
			if (c.$_customer.data("contactInfo") == null ||c.$_customer.data("contactInfo").name ==null || "" === d.val() ) return c.$_customer.removeData("contactInfo"), parent.Public.tips({
				type: 2,
				content: "请选择客户！"
			...
			if (f.length > 0) {
				var g = $.trim(b.$_note.val()),
					h = {
						...
						totalArrears: "",
						sourceOrder : b.$_sourceOrder.val(),
						from : urlParam.from,
...
$(function() {
	...
	if (urlParam.id) {
		if (!hasLoaded) {
			var a = $(".bills").hide();
			if(urlParam.from == "qr"){
				Public.ajaxGet("../scm/invSa/updateInfo?from=qr", {
					id: urlParam.id,
				}, function(b) {
			...
			}else if(urlParam.from == "th"){
				Public.ajaxGet("../scm/invSa/updateInfos?from=th", {
					id: urlParam.id,
				}, function(b) {
					200 === b.status ? (originalData = b.data, THISPAGE.init(b.data), a.show(), hasLoaded = !0) : parent.Public.tips({
						type: 1,
