...
function initValidator() {
	$_form.validator({
		...
		valid: function() {
			var d = parent.$("#grid").jqGrid("getRowData",cRowId).name;
			var a = $.trim($("#name").val());
	...
	delete b.firstLink, Public.ajaxPost("../basedata/contact/" + ("add" == oper ? "add" : "update")+"?type=10", b, function(d) {
		if (200 == d.status) {
			if (parent.parent.Public.tips({
				...
				var e = d.data.id;
				var C = d.data.cCategoryName;
				d = b, d.id = e, d.customerType = C, d.contacter = c.linkName || "", d.mobile = c.linkMobile || "", d.telephone = c.linkPhone || "", d.linkIm = c.linkIm || "", callback(d, oper, window)
			}
...
function initEvent() {
	...
	//categoryCombo.rawData.splice(0,1,'');
	var b = $("#date");
	b.blur(function() {
