define(["jquery", "print"], function(a) {
	function b() {
		...
		j("#conditions-trigger").trigger("click"), 
		j("#filter-fromDate").val(l.beginDate || ""), 
		j("#filter-toDate").val(l.endDate || ""), 
		/*j("#filter-customer input").val(l.customerNo || ""), */
		j("#filter-goods input").val(l.goodsNo || ""), 
		j("#filter-storage input").val(l.storageNo || ""), 
		l.beginDate && l.endDate && (j("#selected-period").text(l.beginDate + "至" + l.endDate + " " + j("#filter-customer input").val()),
		j("div.grid-subtitle").text("日期: " + l.beginDate + " 至 " + l.endDate)), 
		j("#filter-fromDate, #filter-toDate").datepicker(), Public.dateCheck();
		var a = parent.SYSTEM;
		...
		j("#filter-submit").on("click", function(a) {
			...
			var c = j("#filter-fromDate").val(),
				...
				sup = j("#filter-customer input").val(),
				sto = j("#filter-storage input").val();
			return c && d && new Date(c).getTime() > new Date(d).getTime() ? void parent.Public.tips({
				...
				beginDate: c,
				endDate: d,
				customerNo: j("#filter-customer input").data('numbers') || "",
				goodsNo: j("#filter-goods input").data('numbers') || "",
				storageNo: j("#filter-storage input").data('numbers') || "",
				salesId: j("#filter-saler input").data('numbers') || "",
				profit: b.chkVal().length > 0 ? "1" : "0"
			}, j("#selected-period").text(c + "至" + d + (sup && " " + sup) + (sto && " " + sto)), 
			j("div.grid-subtitle").text("日期: " + c + " 至 " + d),/*j("#filter-menu").removeClass("ui-btn-menu-cur"),*/  void i(+l.profit))
		}), j("#filter-reset").on("click", function(a) {
			a.preventDefault(), 
			j("#filter-fromDate").val(l.beginDate), 
			j("#filter-toDate").val(l.endDate), 
			j("#filter-customer input").data('numbers',"").val(""), 
			j("#filter-goods input").data('numbers',"").val(""), 
			j("#filter-storage input").data('numbers',"").val(""), 
			j("#filter-saler input").data('numbers',"").val(""), 
			b.chkNot()
		})
	}
	function c() {
	...
	function i(a) {
		j(".no-query").remove(), j(".ui-print").show(), "number" == typeof a && (j("#grid").jqGrid(a ? "showCol" : "hideCol", ["unitCost", "cost", "saleProfit", "salepPofitRate"]), resize(), j("#grid").clearGridData(!0)), j("#grid").jqGrid("setGridParam", {
			datatype: "json",
			postData: l,
