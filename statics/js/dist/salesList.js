...
var THISPAGE = {
	...
	loadGrid: function() {
		function a(a, b, c) {
			if(c.sourceType == 2){
				var d = '<div class="operating" data-id="' + c.id + '"><span class="min-btn btn-success ui-icon-pencil" title="查看"><span class="fa fa-edit"></span>查看</span></div>';
			}else{
	...
	addEvent: function() {
		...
		if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
			...
			parent.tab.addTabItem({
				tabid: f,
				text: e,
				url: "../scm/invSa?action=editSale&id=" + b + "&from=" + c.from + "&flag=list" + d + "&transType=" + queryConditions.transType
			})
