var curRow, curCol, loading, urlParam = Public.urlParam(),
	...
	THISPAGE = {
		...
		getArea:function(a){
			...
			var params = {invId:goodsInfo.id,locationId:locationId,num:rowData.qty,transType:"150706",id:originalData.id};
			Public.ajaxPost("../scm/invOi/area", params, function (res) {
				...
				$("#" + a).data("areaInfo",{id:res.areaInfo['id'],name:res.areaInfo['area_name']});
			});
		},
		initDom: function(a) {
			...
			$("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					if ("locationName" === b && ($("#" + d + "_locationName", "#grid").val(c), THISPAGE.storageCombo.selectByText(c)), "batch" === b) {
						var g = $("#" + a).data("goodsInfo"),i = $("#" + a).data("storageInfo") || {};
						if (!g || !i.id) return $("#grid").jqGrid("restoreCell", d, e), curCol = e + 1, void $("#grid").jqGrid("nextCell", d, e + 1);
		...
		_getEntriesData: function(a) {
			...
			for (var b = [], c = $("#grid").jqGrid("getDataIDs"), d = 0, e = c.length; e > d; d++) {
				...
				if ("" !== h.goods) {
					...
					if (!ar.id) return parent.Public.tips({
						type: 2,
						content: "请选择相应的货位！"
