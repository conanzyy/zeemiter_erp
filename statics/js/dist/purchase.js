var curRow, curCol, loading,settlePrice = {}, CONFIG = parent.CONFIG , SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		initData:function(){
			//得到最新结算价
			Public.ajaxGet(CONFIG.SITE_URL + "/scm/invPu/get_settle_price", {}, function(data){
				if(data.status === 200) {
		...
		getArea:function(a){
			...
			Public.ajaxPost("../scm/invPu/area", params, function (res) {
				if(originalData.transType == "150501"){
		...
		initDom: function(a) {
			...
			this.$_note.placeholder(), "150502" == originalData.transType ? (parent.$("#page-tab").find("li.l-selected").children("a").html("采购退货单"), $("#paymentTxt").html("本次退款")) : (parent.$("#page-tab").find("li.l-selected").children("a").html("采购入库单"), $("#paymentTxt").html("本次付款")), this.customerCombo = Business.billSupplierCombo($("#customer"), {}), ("add" !== a.status || a.buId) && (this.$_customer.data("contactInfo", {
				id: a.buId,
				name: a.contactName
			}),
			this.customerCombo.input.val(a.contactName)), hideCustomerCombo && this.customerCombo.disable(),
			...
			var c = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				d = '<a id="edit" class="ui-btn">保存</a><a href=' + CONFIG.SITE_URL + '"/scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				d2 = '<a href=' + CONFIG.SITE_URL + '"/scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				//d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href=' + CONFIG.SITE_URL + '"/scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href=' + CONFIG.SITE_URL +'"/scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				f = "",
				g = "";
		...
		loadGrid: function(a) {
			...
			function p(a) {
				...
				if (b) {
					...
					k && (THISPAGE.calTotal(),THISPAGE.getArea(a));
				}
			}
			var q = this,
				r = (new Date).format();
			...
			$("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					if("locationArea" === b){
						$("#" + d + "_locationArea", "#grid").val(c);
						THISPAGE.areaCombo.selectByText(c);
					}
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost(CONFIG.SITE_URL + "/scm/invPu/add?action=add", {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost(CONFIG.SITE_URL + "/scm/invPu/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost(CONFIG.SITE_URL + "/scm/invPu/updateInvPu?action=updateInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_CHECK")) {
					...
					d && c.ajaxPost(CONFIG.SITE_URL + "/scm/invPu/checkInvPu?action=checkInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost(CONFIG.SITE_URL + "/scm/invPu/revsCheckInvPu?action=revsCheckInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
					...
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: CONFIG.SITE_URL + "/scm/invPu?action=initPur&transType=" + originalData.transType
					})
				}
				...
				a.preventDefault(), Business.verifyRight("PU_PRINT") && Public.print({
					title: "采购单列表",
					$grid: $("#grid"),
					pdf: CONFIG.SITE_URL + "/scm/invPu/toPdf?action=toPdf",
					billType: 150602,
					filterConditions: {
						id: originalData.id
					}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet(CONFIG.SITE_URL + "/scm/invPu/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet(CONFIG.SITE_URL + "/scm/invPu/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
			...
			for (var b = [], c = $("#grid").jqGrid("getDataIDs"), d = 0, e = c.length; e > d; d++) {
				...
				if ("" !== h.goods) {
					...
					if (i) {
						...
						}), $("#grid").jqGrid("editCellByColName", g, "goods"), !1;
...
$(function() {
	...
	if (urlParam.id) {
		if (!hasLoaded) {
			...
			}) : Public.ajaxGet(CONFIG.SITE_URL + "/scm/invPu/update?action=update", {
				id: urlParam.id
			}, function(b) {
