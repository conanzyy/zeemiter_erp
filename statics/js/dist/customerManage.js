...
function initValidator() {
	$_form.validator({
		...
		fields: {
			number: {
				rule: "add" === oper ? "required; type;" : "required; type",
				timely: 3
			},
			name: "required;"
		},
...
function getEntriesData() {
	...
	for (var a = {}, b = [], c = $grid.jqGrid("getDataIDs"), d = !1, e = 0, f = c.length; f > e; e++) {
		...
		if ("" != i.name && "" != i.mobile && "" != i.phone && "" != i.addressStr){
			valid++;
		}
		if ("" != i.name || "" != i.mobile || "" != i.phone || "" != i.addressStr){
			if("" == i.name){
				$("#grid").jqGrid("editCellByColName", h, "name");
				parent.Public.tips({ type: 2, content: "请填写联系人！" });
				return false;
			}
			if("" == i.mobile){
				$("#grid").jqGrid("editCellByColName", h, "mobile");
				parent.Public.tips({ type: 2, content: "请填写手机！" });
				return false;
			}
			if("" == i.phone){
				$("#grid").jqGrid("editCellByColName", h, "phone");
				parent.Public.tips({ type: 2, content: "请填写座机！" });
				return false;
			}
			if("" == i.addressStr){
				$("#grid").jqGrid("editCellByColName", h, "addressStr");
				parent.Public.tips({ type: 2, content: "请填写地址！" });
				return false;
			}
		}
		...
		g = {
			...
			linkPhone: i.phone,
			linkIm: i.im,
			linkAddress: i.addressStr,
			linkFirst: "是" == i.first ? 1 : 0
		};
		var j = $("#" + h).data("addressInfo") || {};
		g.province = j.province, g.city = j.city, g.county = j.county, g.address = j.address, g.id = h , "是" == i.first && (d = !0, a.firstLink = g), b.push(g)
	}
	
	if(valid == 0 ){
		parent.Public.tips({ type: 2, content: "请填写联系信息！" });
		return false;
	}
