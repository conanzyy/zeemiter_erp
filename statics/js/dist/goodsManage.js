...
function init_category(datas){
	var retailPrice = $.parseJSON(datas);
...
function init() {
	void 0 !== cRowId ? Public.ajaxPost("../basedata/inventory/query?action=query", {
		...
		init_category(a.data.retailPrice);
...
function getCustomerData() {	
	...
	if (a) {
		var b = {
			...
			salePriceKh: JSON.stringify(khdata),
...
function getEntriesData() {
	...
	for (var c = 0, d = b.length; d > c; c++) {
		...
		if (e = {
			locationId: h.id,
			quantity: g.quantity,
			unitCost: Public.currencyToNum(g.unitCost == "" ? 0:g.unitCost),
			amount: Public.currencyToNum(g.amount == "" ? 0:g.amount),
			batch: "",
			prodDate: "",
			safeDays: "",
			validDate: ""
...
function initEvent() {
	...
	}).getCombo(),storageCombo.selectByValue($("#storage").data("defItem"))
	
/*	 setTimeout(function(){
		 storageCombo = Public.storageTree($storage,sto_opts);
...
function initValidator() {
	...
	$form.validator({
		...
		fields: {
			...
			maxInventory: "checkInventory;required",
			minInventory:"required",
			length: "number;",
			width: "number;",
			height: "number;",
			weight: "number;"
