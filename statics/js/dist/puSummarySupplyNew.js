define(["jquery", "print"], function(a) {
	function b() {
		...
		i("#filter-fromDate, #filter-toDate").datepicker(), chkboxes = i("#chk-wrap").cssCheckbox(), 
		i("#filter-submit").on("click", function(a) {
			...
			}, i("#selected-period").text(b + "至" + c + (sup && " " + sup) + (sto && " " + sto) ),
			i("div.grid-subtitle").text("日期: " + b + " 至 " + c),
			h(), void i("#filter-menu").removeClass("ui-btn-menu-cur"))
		}), i("#filter-reset").on("click", function(a) {s
			a.preventDefault(), 
			i("#filter-fromDate").val(k.beginDate), 
			i("#filter-toDate").val(k.endDate), 
