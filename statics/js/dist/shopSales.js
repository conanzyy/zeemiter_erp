	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			if (this.$_customer = $("#customer"), this.$_date = $("#date").val(system.endDate), this.$_number = $("#number"), this.$_note = $("#note"), this.$_discountRate = $("#discountRate"), this.$_deduction = $("#deduction"), this.$_discount = $("#discount"), this.$_payment = $("#payment"), this.$_arrears = $("#arrears"), this.$_totalArrears = $("#totalArrears"), this.$_toolTop = $("#toolTop"), this.$_toolBottom = $("#toolBottom"), this.$_paymentTxt = $("#paymentTxt"), this.$_accountInfo = $("#accountInfo"),this.$_paymentInfo = $("#paymentInfo"), this.$_userName = $("#userName"), this.$_modifyTime = $("#modifyTime"), this.$_checkName = $("#checkName"), this.$_customerFree = $("#customerFree"), this.customerArrears = 0, this.$_note.placeholder(), "150602" == originalData.transType) {
				...
				parent.$("#page-tab").find("li.l-selected").children("a").html("手工订单");
				...
				callback: {
					onChange: function(a) {
						if (-1 === this.getValue()) b.chooseAccount();
							...
							c.push({
								wayId: this.getValue(),
							}), b.$_paymentInfo.data("paymentInfo", c).hide(), b.$_payment.removeAttr("disabled").removeClass("ui-input-dis")
						}
					}
				}
			});
			var f = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				g = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a><a id="edit" class="ui-btn">保存</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">打印</a>',
				i = "",
				j = "",
				k = "";
				...
				if (b) {
					if (b.batch || $("#grid").jqGrid("setCell", a, "batch", "&#160;"), b.safeDays || ($("#grid").jqGrid("setCell", a, "prodDate", "&#160;"), $("#grid").jqGrid("setCell", a, "safeDays", "&#160;"), $("#grid").jqGrid("setCell", a, "validDate", "&#160;")), 1 == b.isWarranty && $("#grid").jqGrid("showCol", "batch"), b.safeDays > 0 && ($("#grid").jqGrid("showCol", "prodDate"), $("#grid").jqGrid("showCol", "safeDays"), $("#grid").jqGrid("showCol", "validDate")), !b.price) {
						...
						if (c && c.id) {
							...
							var d = e[c.cCategory] == null ||  e[c.cCategory] == "" ? d : e[c.cCategory];
							b.price = d;
						}
					}
		...
		priceBatch:function(cInfo){
			...
			for(var i = 0;i<ids.length;i++){
				...
				if(b['goods'] == "") continue
				var prices = $.parseJSON(b.retailPrice);
				...
				var n = $("#grid").jqGrid("setRowData", ids[i], b);
				n && THISPAGE.calTotal()
			}
		},
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost("../scm/invSa/addSales", {
				...
				if (b.preventDefault(), Business.verifyRight("SA_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost("../scm/invSa/updateSale", {
						postData: JSON.stringify(d)
					}, function(b) {
				...
				if (a.preventDefault(), Business.verifyRight("SA_ADD")) {
					var b = "手工订单",
						c = "shopSalesQuery-shopSales";
					if ("150602" == originalData.transType) var b = "销售退货单",
						c = "sales-salesBack";
					parent.tab.overrideSelectedTabItem({
						tabid: c,
						text: b,
						url: "../scm/invSa?action=initShopSales&transType=" + originalData.transType
					})
				}
				...
				a.preventDefault(), Business.verifyRight("SA_PRINT") && Public.print({
					title: "手工订单列表",
					$grid: $("#grid"),
					pdf: "../scm/invSa/toPdfSales?action=toPdfSales",
					billType: 10201,
					filterConditions: {
						id: originalData.id
					}
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有上一张了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/updateSales", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/updateSales", {
					id: a.idList[a.idPostion]
				}, function(a) {
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			var a = $(".bills").hide();
			urlParam.turn ? Public.ajaxGet("../scm/invSa/queryDetails?action=queryDetails", {
				id: urlParam.id
			}, function(b) {
			...
			}) : Public.ajaxGet("../scm/invSa/updateSales", {
				id: urlParam.id
			}, function(b) {
