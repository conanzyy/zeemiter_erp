	...
	function c() {
		...
		h("#refresh").click(function(b) {
			...
			j = {
				...
				goodsNo: h("#filter-goods input").data('numbers') || "",
				storage: h("#filter-storage input").data("ids") || "",
				storageNo: h("#filter-storage input").data("numbers") || ""
			...
			Public.ajaxGet("../report/deliverSummary?action=detail", j, function(a) {
				200 === a.status ? (h(".no-query").remove(), h(".ui-print").show(), h(".grid-subtitle").text("日期: "+j.beginDate + "至" + j.endDate), d(a.data), l.close()) : (l.close(), parent.Public.tips({
					type: 1,
					content: msg
				}))
			})
	...
	function d(a) {
		...
		h("#grid").jqGrid({
			...
			gridComplete: function() {
				h("#grid").footerData("set", {
					location: "合计:"
				}), h("table.ui-jqgrid-ftable").find('td[aria-describedby="grid_location"]').prevUntil().css("border-right-color", "#fff")
			}
