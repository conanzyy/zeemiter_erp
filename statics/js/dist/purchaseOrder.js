			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("PO_CHECK")) {
					...
					d && c.ajaxPost("../scm/invPo/checkInvPo?action=checkInvPo", {
						...
						200 === b.status ? (originalData.id = b.data.id, $("#mark").addClass("has-audit"), $("#edit").hide(), a.$_checkName.html(SYSTEM.realName).parent().show(), a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit), $("#turn").html("150502" == a.classes.getValue() ? "生成退货单" : "生成采购单"), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
