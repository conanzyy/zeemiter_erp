	...
	THISPAGE = {
		...
		initDom: function() {
			var a = [{
				...
				b = !0;
			if (this.$_companyName = $("#companyName"), 
				this.$_companyAddress = $("#companyAddress"), 
				this.$_companyTel = $("#companyTel"), 
				this.$_companyFax = $("#companyFax"), 
				...
				this.$_qtyPlaces = $("#qtyPlaces"), 
				this.$_pricePlaces = $("#pricePlaces"), 
				this.$_amountPlaces = $("#amountPlaces"), 
				this.$_valMethods = $("#valMethods"), 
				this.$_requiredCheckStore = $("input[name=requiredCheckStore]"),
				this.$_showLowPrice = $("input[name=showLowPrice]"), 
				this.$_establishForm = $("#establish-form2"), 
				this.$_settlePlaces = $("#settlePlaces"),
				this.$_companyName.val(system.companyName), 
				this.$_companyAddress.val(system.companyAddr), 
				this.$_companyTel.val(system.phone),
				this.$_companyBank.val(system.companyBank),
				this.$_companyCode.val(system.companyCode),
				this.$_companyAccount.val(system.companyAccount),
				
				this.$_companyFax.val(system.fax), 
				this.$_postcode.val(system.postcode), 
				this.$_startDate.val(system.startDate), 
				this.$_currency.val(system.currency), 
				this.$_settlePlaces.val(system.settlePlaces),
				
			this.$_settlePlaces.spinbox({
				value: system.settlePlaces,
				width: 80
			}),this.$_qtyPlaces.spinbox({
			...
			system.requiredCheckStore == 1 && (this.$_requiredCheckStore[0].checked = !0), 
			system.requiredCheckStore == 0 && (this.$_requiredCheckStore[1].checked = !0),
			system.showLowPrice == 1 && (this.$_showLowPrice[0].checked = !0), 
			system.showLowPrice == 0 && (this.$_showLowPrice[1].checked = !0),
			2 === siType && (a.push({
			...
			$("#save").click(function() {
				if (Business.verifyRight("SYSTEM_UPDATE")) {
					...
					var g = {
						companyName: b,
						companyAddr: c,
						companyBank:a.$_companyBank.val(),
						companyCode:a.$_companyCode.val(),
						companyAccount:a.$_companyAccount.val(),
						phone: d,
						fax: e,
						postcode: f,
						settlePlaces: sp,
						...
						requiredCheckStore: $("input[name=requiredCheckStore]:checked").val(),
						showLowPrice: $("input[name=showLowPrice]:checked").val()
					};
