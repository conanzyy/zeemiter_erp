	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var d = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				...
				g = "",
				h = "",
				i = "150602" == originalData.transType ? "生成退货单" : "生成销售单",
			...
			this.customerCombo.input.enterKey(), this.$_date.bind("keydown", function(b) {
				...
				if (b.preventDefault(), Business.verifyRight("SO_CHECK")) {
					...
					d && c.ajaxPost("../scm/invSo/checkInvSo?action=checkInvSo", {
						...
						200 === b.status ? (originalData.id = b.data.id, $("#mark").addClass("has-audit"), $("#edit").hide(), a.$_checkName.html(SYSTEM.realName).parent().show(), a.disableEdit(), $("#groupBtn").html(a.btn_turn + a.btn_view + a.btn_reaudit), $("#turn").html("150602" == a.classBox.getValue() ? "生成退货单" : "生成销售单"), parent.Public.tips({
							content: "审核成功！"
						})) : parent.Public.tips({
							type: 1,
							content: b.msg
