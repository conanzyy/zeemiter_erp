...
var THISPAGE = {
	...
	loadGrid: function(a) {
        ...
        //结算号下拉
        function es(){
            var a  = $(".settlementAuto")[0];
            return a
        }
        function fs(a,b,c){
            if("get"===b){
                if ("" !== $(".settlementAuto").getCombo().getValue()) return $(a).val();
                    var d = $(a).parents("tr");
                    return d.removeData("settlementInfo"), ""
            }
        }
        function gs(){
            $("#initCombo").append($(".settlementAuto").val(""))
        }
		...
		$("#accountGrid").jqGrid({
			...
			colModel: [{
                ...
                editoptions: {
					...
					custom_value: fs,
					handle: gs,
					trigger: "ui-icon-ellipsis settlement-trigger"
				}
			}, {
				name: "remark",
				label: "备注",
			...
			afterEditCell: function(a, b, c, d) {
				THISPAGE.curID = a;
				"accName" === b && ($("#" + d + "_accName", "#accountGrid").val(c),h.accountCombo.selectByText(c)); 
				"wayName" === b && $("#" + d + "_wayName", "#accountGrid").val(c);
	...
	initCombo: function() {
		...
		}), Business.settlementCombo($(".settlementAuto"),{
            callback: {
				onChange: function(a) {
					var b = this.input.parents("tr");
					a && b.data("settlementInfo", a)
				}
			}
        })
	},
			...
			if (Business.verifyRight("PAYMENT_ADD")) {
				...
				d && (c.addClass("ui-btn-dis"), Public.ajaxPost("../scm/payment/addNew?action=addNew", {
					...
					if (c.removeClass("ui-btn-dis"), 200 === b.status) {
						...
						for (var d = 1; 8 >= d; d++) $("#accountGrid").jqGrid("addRowData", d, {});
	...
	resetData: function() {
		var a = this;
		$("#grid").clearGridData();
		for (var b = 1; 5 >= b; b++) $("#grid").jqGrid("addRowData", b, {}), $("#grid").jqGrid("footerData", "set", {
			qty: 0,
			amount: 0
		});
