...
$(function(){
	...
	//新增分录
	$('.button-row').on('click', '.ui-icon-plus', function(e){
		$(this).parent().parent().click();
		var rowId = $('#grid tbody tr:last').find('div.operating').data('id');
		var newId = $('#grid tbody tr').length;
		var datarow = { id: newId };
		var su = $("#grid").jqGrid('addRowData', newId, datarow, 'after', rowId);
		if(su) {
 ...
 * 报表自适应
 */
Public.resizeReport = function(offset){
	var grid = $("#grid");
	var oldHiehgt = grid.height();
	var oldWidth = grid.width();
	var width = $(window).width() - 30 * 2 - 2;
	var height = $(window).height() - $("#grid-layout").offset().top - 15 * 2 - (offset || 0) - 3;
	if(oldHiehgt < height) {
		height = oldHiehgt;
		if(oldWidth>width) height = height + 17
	}
...
Public.billsOper = function (val, opt, row) {
	var html_con = '<div class="operating" data-id="' + opt.rowId + '"><span class = "min-btn ui-icon-plus btn-success"><span class="fa fa-plus" title="新增行"></span>新增</span> <span class = "min-btn ui-icon-trash btn-warning"><span class="fa fa-trash" title="删除行"></span>删除</span>';
	return html_con;
};
Public.billsOper_goods = function (val, opt, row) {
	var html_con = '<div class="operating" data-id="' + opt.rowId + '"><span class = "min-btn ui-icon-plus btn-success"><span class="fa fa-plus" title="新增行"></span>新增</span> <span class = "min-btn ui-icon-trash btn-warning"><span class="fa fa-trash" title="删除行"></span>删除</span> <span class = "min-btn ui-icon-cart btn-info"><span class="fa fa-search" title="商品库存查询"></span>库存查询</span></div>';
	return html_con;
};
...
Public.Tips.prototype = {
	...
	_create : function(){
		...
		switch(opts.type){
			case 0 :
				this.obj.addClass('ui-tips-success').find("i").addClass("fa fa-check");
				break ;
			case 1 :
				this.obj.addClass('ui-tips-error').find("i").addClass("fa fa-warning");;
				break ;
			case 2 :
				this.obj.addClass('ui-tips-warning').find("i").addClass("fa fa-warning");
				break ;
			default :
...
Business.stockTypeCombo = function ($_obj, opts){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			return [{id:'1',name:'总仓备'},{id:'2',name:'子仓备'}];
		},
		width: 120,
		height: 300,
		text: 'name',
		...
		editable: true,
		emptyOptions: false,
		addOptions : {text : '空',value : ''},
	}, opts);

	var stockCombo = $_obj.combo(opts).getCombo();
...
//车型品牌
Business.BrandsCombo = function($_obj, opts){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.BrandsInfo){
				return defaultPage.SYSTEM.BrandsInfo;
			}else{
				return '../basedata/employee?action=ModelsAll';
			}
		},
		ajaxOptions: {
			formatData: function(data){
				defaultPage.SYSTEM.BrandsInfo = data.data.items.brands;
				defaultPage.SYSTEM.CarsInfo = data.data.items.cars;	//更新
				defaultPage.SYSTEM.ModelInfo = data.data.items.models;
				defaultPage.SYSTEM.YearlInfo = data.data.items.years;
				defaultPage.SYSTEM.DisInfo = data.data.items.dis;
				return data.data.items.brands;
			}
		},
		width: 120,
		height: 300,
		...
		editable: true,
		emptyOptions: false,
		addOptions : {text : '汽车品牌',value : '',rawData:{text : '汽车品牌',value : ''}},
		callback: {
			onChange: function(data){
				if(data != null){
					$cars = Business.CarsCombo($("#matchCars"), { width: 170},brandsCombo);
					$cars.selectByIndex(0,true);
	...
	var brandsCombo = $_obj.combo(opts).getCombo();
	return brandsCombo;
};

Business.CarsCombo = function($_obj, opts , parent){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.CarsInfo){
				if(parent.getValue() == null || parent.getValue() == ''){
					return [];
				}
				return defaultPage.SYSTEM.CarsInfo[parent.getValue()];
			}else{
				return '../basedata/employee?action=ModelsAll';
			}
		},
		...
		cache: false,
		editable: true,
		addOptions : {text : '汽车车系',value : '',rawData:{text : '汽车车系',value : ''}},
		callback: {
			onChange: function(data){
				if(data != null){
					$models = Business.ModelsCombo($("#matchModel"), { width: 170 },carsCombo);
					$year = Business.YearsCombo($("#matchYear"), { width: 170 },carsCombo);
					$dis = Business.DisplacementsCombo($("#matchDisplacement"), { width: 170 },carsCombo);
					$models.selectByIndex(0,true);
					$year.selectByIndex(0,true);
					$dis.selectByIndex(0,true);
				}
			}
		},
	}, opts);
	var carsCombo = $_obj.combo(opts).getCombo();

	return carsCombo;
};
Business.ModelsCombo = function($_obj, opts){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.ModelInfo){

				return defaultPage.SYSTEM.ModelInfo;
			}else{
				return '../basedata/employee?action=ModelsAll';
			}
		},
		...
		editable: true,
		emptyOptions:false,
		addOptions : {text : '汽车车型',value : '',rowData:{text : '汽车车型',value : ''}},
		callback: {
			onChange: function(data){
				if(data != null){
					$year = Business.YearsCombo($("#matchYear"), { width: 170 },modelsCombo);
					$dis = Business.DisplacementsCombo($("#matchDisplacement"), { width: 170 },modelsCombo);
					$year.selectByIndex(0,true);
					$dis.selectByIndex(0,true);
				}
			}
	...
	var modelsCombo = $_obj.combo(opts).getCombo();

	return modelsCombo;
};

Business.YearsCombo = function($_obj, opts, parent){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.YearInfo){
				if(parent.getValue() == null || parent.getValue() == ''){
					return [];
				}
				return defaultPage.SYSTEM.YearInfo[parent.getValue()];
			}else{
				return '../basedata/employee?action=ModelsAll';
			}
		},
		ajaxOptions: {
			formatData: function(data){
				...
				defaultPage.SYSTEM.CarsInfo = data.data.items.cars;	//更新
				defaultPage.SYSTEM.ModelInfo = data.data.items.models;
				defaultPage.SYSTEM.YearInfo = data.data.items.years;
				defaultPage.SYSTEM.DisInfo = data.data.items.dis;
				return data.data.items;
			}
		},
	...
	var yearsCombo = $_obj.combo(opts).getCombo();

	return yearsCombo;
};
Business.DisplacementsCombo = function($_obj, opts , parent){
	if ($_obj.length == 0) { return };
	var defaultPage = Public.getDefaultPage();
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.DisInfo){
				if(parent.getValue() == null || parent.getValue() == ''){
					return [];
				}
				return defaultPage.SYSTEM.DisInfo[parent.getValue()];
			}else{
				return '../basedata/employee?action=ModelsAll';
			}
		},
	...
	var displacementsCombo = $_obj.combo(opts).getCombo();

	return displacementsCombo;
};
				...
				if(data) {
					if(data.id != goodsInfo.id){
						...
						if(!opts.disSerNum  && SYSTEM.ISSERNUM && data.isSerNum){
							...
							//设置默认仓库
							if(defaultStorage){
								data.locationId = data.locationId == null || data.locationId == '' || data.locationId == 0 ? defaultStorage.id : data.locationId;
								data.locationName = data.locationName == null || data.locationName == '' ? defaultStorage.name : data.locationName;
							}
...
Business.goodsCombo = function($_obj, opts){
	...
	$('.quickAddGoods').unbind('click').on('click', function(e){
		...
		var _w = 775,
		_h = 480;//defaultPage.SYSTEM.enableStorage?480:370;
...
/*					if(!storageInfo) {
					...
					if(data) {
						...
						if (defaultPage.SYSTEM.storageAreaInfo) {
							var areas =  defaultPage.SYSTEM.storageAreaInfo[data.id];
							var aa = areas[0];
							$("#grid").jqGrid('setRowData', parentTr.attr('id'), {'locationArea':aa.name});
							parentTr.data('areaInfo', { id: aa.id, name: aa.name });
						}
					}
				}
			}
...
Business.areaCombo = function($_obj, opts){
	var defaultPage = Public.getDefaultPage();
	if ($_obj.length == 0) { return };
	var opts = $.extend(true, {
			data: function(){
				if(typeof this.input=='undefined'){
					...
					var storagedata = this.input.parents('tr').data('storageInfo');
					if(storagedata==null){
						parent.Public.tips({
							type: 1,
							content: "请选择仓库!"
					...
					if (defaultPage.SYSTEM.storageAreaInfo) {
						var areas =  defaultPage.SYSTEM.storageAreaInfo[storagedata.id];
						return areas;
					}
				}
			},
	...
	var areaCombo = $_obj.combo(opts).getCombo();
	return areaCombo;
};
...
//结算号
Business.settlementCombo = function($_obj, opts){
	if ($_obj.length == 0) { return };
	var opts = $.extend(true, {
		data: function(){
			return [];
		...
		ajaxOptions: {
			formatData: function(data){
				SYSTEM.infosptyInfop = data.data.items;	//更新缓存
				return data.data.items;
			}
		},
		/*emptyOptions: true,*/
	...
	var settlementCombo = $_obj.combo(opts).getCombo();
	return settlementCombo;
};




Business.orderTypeCombo = function($_obj, opts){
	var opts = $.extend(true, {
		data: function(){
			if(SYSTEM.orderTypeInfo) {
				return SYSTEM.orderTypeInfo[opts['type']];
			} else {
				return '../basedata/orderType?action=orderType&isDelete=2&';
			}
		},
		ajaxOptions: {
			formatData: function(data){
				SYSTEM.orderTypeInfo = data.data.items;	//更新缓存
				return data.data.items;
			}
		},
		//emptyOptions: true,
	...
	var orderTypeCombo = $_obj.combo(opts).getCombo();
...
Business.billsEvent = function(obj, type, flag){
	...
	$('.grid-wrap').on('click', '.ui-icon-plus', function(e){
		...
		$(this).parent().parent().click();//触发修改事件。
		var newId = $('#grid tbody tr').length;
	...
	if(type !== 'assemble') {
		$('#customer').on('click', '.ui-icon-ellipsis', function(e){
			...
			_self.customerDialog = $.dialog({
				...
				lock: true,
				button:[{name: '确认',defClass:'ui_state_highlight ui-btn-confirm ui-btn', callback: function () {
//							window.location.reload();
						...
						{name: '关闭',defClass:'ui-btn', callback: function () {
					        return true;
						}}]
				/*ok: function(){
					if(typeof this.content.callback === 'function'){
		...
		$(".grid-wrap").on("click", ".settlement-trigger", function() {
			var defaultPage = Public.getDefaultPage();
			var skuMult = false;
			if(!$(this).hasClass('disableSku')){
				skuMult = $(this).data('skuMult') || defaultPage.SYSTEM.enableAssistingProp;
			}
			/*alert(JSON.stringify($("#grid").jqGrid('getRowData')));
			return;*/
			var contact = $("#customer").find('input').val();
			contact = contact.split(' ')[0];
			if(contact == '(空)' || contact == '' || contact == null){
				parent.Public.tips({
					type : 2,
					content : '请选择客户！'
				});
			...
			$.dialog({
				...
				height: 410,
				title: '单据查询',  
				content: 'url:../inventory?action=orderList&transType='+transType+'&contact='+contact,
				data: {
					skuMult: skuMult,
					skey:_self.skey,
					callback: function(newId, curID, curRow){
						if(curID === '') {
							$("#accountGrid").jqGrid('addRowData', newId, {}, 'last');
							_self.newId = newId + 1;
						};
						setTimeout( function() { $("#accountGrid").jqGrid("editCell", curRow, 2, true) }, 10);
						_self.calTotal();
					}
				},
				lock: true,
				button:[{name: '选中',defClass:'ui_state_highlight ui-btn fl', callback: function () {
					this.content.callback();
			        return false;
				}},
				{name: '选中并关闭',defClass:'ui_state_highlight ui-btn', callback: function () {
					this.content.callback();
					this.close();
			        return false;
				}},
		...
		$('.grid-wrap').on('click', '.goods .ui-icon-storage-search', function(e){
			var skuMult = false;
			if(!$(this).hasClass('disableSku')){
				skuMult = $(this).data('skuMult') || defaultPage.SYSTEM.enableAssistingProp;
			}
			$.dialog({
				...
				height: 410,
				title: '仓库商品选择',
				content: 'url:../inventory?action=listAll',
				data: {
					skuMult: skuMult,
					skey:_self.skey,
					callback: function(newId, curID, curRow){
		...
		$('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
			...
			//alert($("#customer").find('input').val());
			//console.log($("#customer input"));
			$height = 530
			if(bussType == 'out'){
				 var url = 'url:../settings/goods_batch_all?bussType='+bussType+'&transType='+urlParam.transType;
			     $height = $height + 30;
			}else{
				$buId = $("#customer").data("contactInfo").id;
				if($buId == '1' || $buId == '2'){
					var url = 'url:../settings/goods_batch_kz?&bussType='+bussType
				}else{
					var url = 'url:../settings/goods_batch?bussType='+bussType
				}
			}
	...
	$('.wrapper').on('click', '#storageBox li', function(e){
		...
		var araName = 'locationArea'
		var araInfo = 'areaInfo';
...
Business.filterCustomer = function(){
	Business.customerCombo($('#customerAuto'), {
		...
		callback: {
			onChange: function(data){
				if(data) {
					this.input.data('ids', data.id).data('numbers',data.number);
	...
	$('#filter-customer .ui-icon-ellipsis').on('click', function(){
		...
		$.dialog({
			...
			lock: true,
			ok: function(){
				Business.setFilterData(this.content, $input);this.close();
			},
			cancel: function(){
				return true;
			}
...
//			content: 'url:../settings/goods_batch',
			content: 'url:../settings/goods_batch_all?bussType=out',
			data:{
				width:$width,
...
Business.filterStorage = function(){
	...
	Business.storageCombo($('#storageAuto'), {
		...
		callback: {
			onChange: function(data){
				if(data) {
					this.input.data('numbers', data.locationNo).data('ids', data.id);;
					this.input.val(data.name);
				}
			}
		}
...
Business.filterSaler = function(){
	Business.salesCombo($('#salerAuto'), {
			...
			callback: {
				onChange: function(data){
					if(data) {
						this.input.data('numbers', data.number).data('ids', data.id);;
						this.input.val(data.name);
					}
				}
			},
...
Business.setFilterData = function(dialogCtn, $input){
	...
	$input.data('ids', ids.join(',')).data('numbers', numbers.join(',')).val(names.join(','));
};
...
Public.print = function (opt){
	...
	if((voucherIds == null || voucherIds  == '') && (opt.filterConditions.id == null || opt.filterConditions.id == ""))
	{
		Public.tips({
	        type: 2,
	        content: "请先勾选单据！"
...
Public.zTree_kz = {
    ...
    init: function($target, opts, setting ,callback) {
        ...
        self.container = $($target);
        self.obj = $(self._getTemplate(opts));
        self.container.append("<div class='padd'><div class='input-append row-fluid'><input id='search_condition' type='text' placeholder='请输入搜索条件' class='span8'/><button type='button' class='btn btn-info' onclick=search_ztree(&apos;"+self.obj.attr('id')+"&apos;,&apos;search_condition&apos;)>搜索</button></div></div>");
        self.container.append(self.obj);
        setting = $.extend(true, self.setting, setting);
        Public.ajaxPost(opts.url || '../basedata/assist?action=kzlist&typeNumber=trade&isDelete=2', {}, function(data) {
            if (data.status === 200 && data.data) {
            	self._callback(data.data.items);
            } else {
            	Public.tips({
...
function setFontCss_ztree(treeId, treeNode) {
    if (treeNode.id == 0) {
        ...
        //叶子节点
        return (!!treeNode.highlight) ? {color:"#ee3232", "font-weight":"bold"} : {color:"#333", "font-weight":"normal"};
    } else {
        //父节点
        return (!!treeNode.highlight) ? {color:"#ee3232", "font-weight":"bold"} : {color:"#333", "font-weight":"normal"};
    }
	...
	var _Combo = {
		...
		showTree:function(){
			...
			if(this.zTree){
				...
				this.zTree.obj.css({width:w, top:offset.top + topDiff,left:offset.left-1});
			}
...
//分类下拉框
Public.storageTree = function($obj, opts) {
	if ($obj.length === 0) {
        return;
    }
	opts = opts ? opts : opts = {};
	...
	var _Combo = {
		...
		init : function(){
			...
			this.zTree = _zTree.init($('body'), opts , opts.treeSettings);
			return this;
		},
		showTree:function(){
			if(!this.zTree)return;
	...
	var _zTree = {
		    ...
		    _getTemplate: function(opts) {
		        ...
		        return '<ul id="'+this.id+'" class="zee_tree ' + _defaultClass + '"></ul>';
		    },
		    init: function($target, opts, setting ,callback) {
		        if ($target.length === 0) {
		            return;
		        ...
		        self.container.append(self.obj);
		        setting = $.extend(true, self.setting, setting);
		        //alert(defaultPage.SYSTEM.storageInfo);
				...
				if(g.colModel){
					for (var j = 0; j < g.colModel.length; j++) {
						...
						if(c1['name'] === c2['name']){
							...
							defColModel[i] = _modelClone(c1 , c2);
						}
					};
				}
			};
